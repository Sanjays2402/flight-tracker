...
import TopcFmsEntry from './topc-fms-entry'
...
import ExtltExteriorLighting from './extlt-exterior-lighting'
...
import UerfRotorBurst from './uerf-rotor-burst'
import GasaGoaround from './gasa-goaround'
import AsipAging from './asip-aging'
import RaasRunwayAware from './raas-runway-aware'
import AidcInterfacility from './aidc-interfacility'
...
import ApbnApproachBan from './apbn-approach-ban'
import EicasCascade from './eicas-cascade'
import UprtUpset from './uprt-upset'
...
import HzmtDangerousGoods from './hzmt-dangerous-goods'
import FodRunwayContamination from './fod-runway-contamination'
...
export default function FlightMap() {
  ...
  const [showTopc, setShowTopc] = useState<boolean>(() => lsGet('ft-topc', false))
  ...
  const [showHzmt, setShowHzmt] = useState<boolean>(() => lsGet('ft-hzmt', false))
  const [showFod, setShowFod] = useState<boolean>(() => lsGet('ft-fod', false))
  ...
  const [showApbn, setShowApbn] = useState<boolean>(() => lsGet('ft-apbn', false))
  const [showEicas, setShowEicas] = useState<boolean>(() => lsGet('ft-eicas', false))
  const [showUprt, setShowUprt] = useState<boolean>(() => lsGet('ft-uprt', false))
  const [showTls, setShowTls] = useState<boolean>(() => lsGet('ft-tls', false))
  const [showVaman, setShowVaman] = useState<boolean>(() => lsGet('ft-vaman', false))
  ...
  const [showExtlt, setShowExtlt] = useState<boolean>(() => lsGet('ft-extlt', false))
  ...
  const [showUerf, setShowUerf] = useState<boolean>(() => lsGet('ft-uerf', false))
  const [showGasa, setShowGasa] = useState<boolean>(() => lsGet('ft-gasa', false))
  ...
  const [showAidc, setShowAidc] = useState<boolean>(() => lsGet('ft-aidc', false))
...
<Placemark><name>Current</name><Point><coordinates>${selected.lng},${selected.lat},${selected.altitudeFt*0.3048}</coordinates></Point></Placemark>
      ...
      {showTopc && (
        <TopcFmsEntry
          map={mapRef.current}
          flights={flights.map(f => ({ icao: f.icao, callsign: f.callsign, type: f.type, operator: f.operator, category: f.category, lat: f.lat, lng: f.lng, altitudeFt: f.altitudeFt, velocityKts: f.velocityKts, track: f.track, vertRate: f.vertRate, ground: f.ground }))}
          onClose={() => { setShowTopc(false); lsSet('ft-topc', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 8) } }}
        />
      )}
      ...
      {showHzmt && (
        <HzmtDangerousGoods
          map={mapRef.current}
          flights={flights.map(f => ({ icao: f.icao, callsign: f.callsign, type: f.type, operator: f.operator, lat: f.lat, lng: f.lng, alt: f.altitudeFt, vs: f.vertRate, gs: f.velocityKts, trk: f.track }))}
          onClose={() => { setShowHzmt(false); lsSet('ft-hzmt', false) }}
          onFlyTo={(lat, lng, zoom) => flyToLatLng(lat, lng, zoom)}
        />
      )}

      {showFod && (
        <FodRunwayContamination
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowFod(false); lsSet('ft-fod', false) }}
      ...
      {showPnr && (
        <PnrMonitor
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowPnr(false); lsSet('ft-pnr', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 5) } }}
        />
      )}
      ...
      {showBtv && (
        <BtvMonitor
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowBtv(false); lsSet('ft-btv', false) }}
      ...
      {showApbn && (
        <ApbnApproachBan
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowApbn(false); lsSet('ft-apbn', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 11) } }}
        />
      )}
      {showEicas && (
        <EicasCascade
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowEicas(false); lsSet('ft-eicas', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 9) } }}
        />
      )}
      {showUprt && (
        <UprtUpset
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowUprt(false); lsSet('ft-uprt', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 9) } }}
        />
      )}
      {showTls && (
        <TlsReichCrm
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowTls(false); lsSet('ft-tls', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 9) } }}
        />
      )}
      {showVaman && (
        <VaManRudder
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowVaman(false); lsSet('ft-vaman', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 9) } }}
        />
      )}
      {showBlkhol && (
      ...
      {showExtlt && (
        <ExtltExteriorLighting
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowExtlt(false); lsSet('ft-extlt', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 8) } }}
        />
      )}
      {showDutchRoll && (
      ...
      {showBirdx && (
        <BirdxStrike
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowBirdx(false); lsSet('ft-birdx', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 11) } }}
        />
      )}
      {showScrm && (
        <ScrmSterileCockpit
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowScrm(false); lsSet('ft-scrm', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 9) } }}
        />
      )}
      {showFreq && (
      ...
      {showApchCat && (
        <ApchCatIls
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowApchCat(false); lsSet('ft-apchcat', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 10) } }}
        />
      )}
      {showCvfp && (
        <CvfpChartedVisual
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowCvfp(false); lsSet('ft-cvfp', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 10) } }}
        />
      )}
      {showRdp && (
      ...
      {showUerf && (
        <UerfRotorBurst
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowUerf(false); lsSet('ft-uerf', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 8) } }}
        />
      )}
      {showGasa && (
        <GasaGoaround
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowGasa(false); lsSet('ft-gasa', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 8) } }}
        />
      )}
      {showAsip && (
      ...
      {showAidc && (
        <AidcInterfacility
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowAidc(false); lsSet('ft-aidc', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 6) } }}
        />
      )}
      {showCrda && (
        <CrdaGhostTarget
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowCrda(false); lsSet('ft-crda', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 13) } }}
        />
      )}
      {showPincap && (
        <PilotIncap
          map={mapRef.current}
          flights={flights as any}
          onClose={() => { setShowPincap(false); lsSet('ft-pincap', false) }}
          onFly={(icao) => { const f = flightsRef.current.find(x => x.icao === icao); if (f) { setSelected(f); flyToLatLng(f.lat, f.lng, 9) } }}
        />
      )}
      {showEmas && (
          ...
          <div className="mt-16 mr-4 w-[min(92vw,420px)] max-h-[78vh] overflow-y-auto bg-slate-950/95 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl" onClick={e=>e.stopPropagation()}>
            ...
            {[
              ...
              {group:'Safety & Traffic', items:[
                ...
                ['BIRDX · Bird-Strike / Wildlife-Ingestion Engine-Damage Risk Monitor · per-airframe live evaluator of every climb/approach aircraft\'s exposure to ICAO Annex 14 Vol I §9.4 wildlife-strike risk fused with 14 CFR §33.76 / EASA CS-E 800 single-large-bird (1.85/2.50/3.65 kg by inlet-area class) and medium-flock (0.45-1.15 kg) ingestion-cert envelope margin per airframe-engine-phase-flyway tuple, anchored to the FAA Wildlife Strike Database 2024 Report (Dolbeer/Begier/Wright/Schreckengost/Pfeiffer · 273,769 strikes 1990-2023 · $957M annual loss · 92% strikes below 3000ft AGL · peak <1500ft AGL initial-climb/final-approach phases) and the canonical NTSB AAR-10-03 US Airways 1549 KLGA Hudson 2009-01-15 dual-CFM56-5B4/P stall after Canada Goose (~3.6 kg) ingestion at 2818ft AGL precedent — the seminal §33.76 large-bird breach case that drove cert-amdt-26 envelope from 1.85 to 2.50/3.65 kg for medium and large turbofan classes · structurally distinct from HAIL (frozen-precip ice-impact §33.78 separate cert family different ingestion mechanism), FOD (loose-debris runway-sweep regime AC 150/5210-24), CAST (top-accident category taxonomy CICTT), VOLCANIC-ASH (silicate erosion distinct ingestion mechanism IAVW VAAC), NVPM (engine non-volatile-PM emissions ICAO Annex 16 Vol II), DEEPSTL (post-stall T-tail pitch authority §25.203), MACH-TUCK (transonic compressibility §25.255), MELT (gross-weight winter inversion) — BIRDX is uniquely the WILDLIFE-INGESTION engine-damage-risk evaluator scoring (a) per-flyway corridor placement and (b) seasonal migration peak activity and (c) diel dawn/dusk window and (d) airport hazard rate × WHMP maturity (FAR 139.337 / AC 150/5200-32B) and (e) §33.76 cert-envelope margin per host engine type · 12-flyway global migratory-corridor catalogue spanning the four USFWS North American flyways Atlantic/Mississippi/Central/Pacific per NAWMP 2018 wing-band recoveries Lincoln estimators + Boere Waterbird Populations 2007 + AEWA/EAAFP Partnership 2024: ATL-NA (peak Mar-May Sep-Nov LARGE waterfowl) / MIS-NA (peak Mar-May Sep-Nov LARGE peak 1.00 highest) / CEN-NA (Mar-Apr Sep-Nov MIXED) / PAC-NA (Klamath Basin staging Mar-Apr Sep-Nov LARGE) / EAA-EU East-Atlantic AEWA / MED-EU Black Sea/Mediterranean peak 0.93 / WAS-AS West Asian / EAA-PAC East Asian-Australasian Yellow-Sea staging peak 0.95 / CAS-AS Central Asian CMS CAF / PAC-AM Pacific Americas WHSRN / ATL-AM Atlantic Americas / AUS-NZ Australasian local · 22-airport wildlife-hazard catalogue ranked by FAA Strike DB strike-count + WHMP plan maturity 0-1 (1 = full FAR 139.337 plan + USDA Wildlife Services biologist) + species hazard profile WATERFOWL/RAPTOR/GULL/PASSERINE/MIXED: KJFK rate 0.96 WHMP 0.95 WATERFOWL Jamaica Bay NWR adj PANYNJ Goose Cull · KLGA 0.92 Hudson 1549 NTSB AAR-10-03 precedent · KEWR NJ Meadowlands · KORD Lake MI gull-roost · KDEN Rocky-Mtn raptor · KSLC GSL avian fly-zone · KSFO SF Bay NWR · KBOS Eastern 375 L188 Logan 1960 PIB · KMIA Everglades · KLAX Ballona · KDFW · KATL · EGLL Heathrow CAA CAP 772 · LFPG DSAC PERIL falconry · EHAM Turkish 1951 KLM 738 2009 NLD AAR · EDDF BFU · LIRA Ciampino Ryanair FR4102 starling 2008-11-10 ANSV · LEMD AESA · RJAA EAAFP Yellow-Sea · RJTT JCAB Tokyo Bay gull · ZBAA CAAC · YSSY CASA MOS 139 §10.16 · 8-class §33.76/§33.77 engine-ingestion cert catalogue CFM56 (NB legacy B737NG/A320ceo med-cert 0.70 kg / lrg-cert 1.85 kg / containment 0.86 / amdt-23) / V2500 (A320ceo IAE TC / 0.70/1.85/0.84) / LEAP-1A/1B (A320neo/B737MAX 0.85/2.50/0.92 amdt-26) / PW1100G GTF (A320neo/A220 0.85/2.50/0.90 amdt-26) / GEnx-1B / Trent 1000 (B787/A330neo 1.15/2.75/0.94) / Trent XWB (A350-900/1000 1.15/3.65/0.95 §33.76(d) large-bird 3.65kg) / GE9X (B777X 1.15/3.65/0.96 §33.76(d)) / PW127/PT6 (AT72/Q400/DH8 turboprop §33.77 fan-blade 0.45/1.15/0.55 SAE ARP1542) · 8-phase classifier CLB-INI (post-rotation through 1500ft AGL peak Hudson-mode 1.45 weight) / CLB (1500-10000ft AGL 1.18) / CRZ (above FL100 0.45 strike-rate <1%) / DSC (0.80) / APP-INT (3000-1500ft AGL 1.25) / APP-FNL (<1500ft AGL Ryanair FR4102-mode 1.40) / TXY (0.55) / OFF · 8 drivers PHASE (climb/approach phase weight) / AGL (peak <1500ft per FAA NWSD 92% strikes <3000ft) / FLYWAY (corridor placement and seasonal peak) / SEASON (Mar-May / Aug-Nov migration months) / DIEL (dawn/dusk raptor + waterfowl activity 0-1 from local solar) / AIRPORT (FAA Strike DB rate × WHMP maturity within 12NM snap) / MASS (species mass distribution per flyway-dominant-mass SMALL/MED/LARGE/XL) / CERT (§33.76 cert-mass / actual-bird-mass ratio — <1 = cert-envelope breach) with composite max·0.62 + mean·0.38 × phase-weight × ADV-MUL clipped [0,100] · hard escalators (a) cert margin <0.55 + CLB-INI/APP-FNL → 92 §33.76 envelope breach in low-AGL critical phase (b) KJFK/KLGA/EHAM rate≥0.90 + MIG-active + CLB → 85 Hudson-mode (c) XL goose-class (≥2.5 kg) + CLB-INI → 78 post-rotation dual-ingest · 6-tier classification CRIT-DMG ≥85 rose §33.76 envelope BREACHED expect engine damage if struck brief NO LAND-DAMAGE-INSPECTED per Boeing FCOM ENG.SUPP / Airbus PRO-ABN-70 / HIGH-DMG ≥65 rose-pink above cert envelope monitor TCAS for flock + lookout / DAMAGE ≥45 amber within envelope but damage probable arm CONTINUOUS-IGNITION per FCOM SUPP / WATCH ≥25 sky elevated flyway-density / dusk brief crew lookout use landing-lights below 10kAGL FAA AC 91-44 §3 / NOMINAL <25 emerald low strike-risk band / OFF slate on-stand or cruise FL>200 · MapLibre overlay rendering 7 layers — birdx-fly-fill active-month flyway polygons (rose-active or slate-off-season at 5%/2% opacity) / birdx-fly-line dashed perimeter / birdx-apt airport-hazard sized-coloured circle (rate≥0.85 rose / 0.70 rose-pink / 0.55 amber / sky) at 30% opacity / birdx-apt-lbl IATA+species-profile / birdx-halo per-aircraft tier-colour halo sized 6-24px by score / birdx-pin score≥65 dot / birdx-lbl callsign+engine-class+bird-species+mass+phase label · 4-tab panel AIRCRAFT (per-airframe row with phase/engine-class/AGL/flyway/bird-mass/cert-margin/airport-snap/WHMP/diel/8-driver chiplets/tier-advice) / FLYWAYS (per-flyway aggregate with id/name/migration-active/dominant-mass/traffic/critical-count/damage-count/μ-score/peak%/peak-months/citation) / ENGINES (per-class aggregate with id/label/fleet-count/critical-count/μ-cert-margin/med-cert/lrg-cert/containment/exemplars/μ-score/citation) / METHOD (regulatory regime + scoring model + hard escalators + data sources + precedent accident family + distinct-from + references) · controls ADV-MUL 50-200% / SCOPE 3-40NM airport-snap radius / MAX-FL 30-250 strike-envelope ceiling / MO AUTO/Mar/May/Sep/Nov month-override for seasonal migration / per-tier filter / per-phase filter (5 phases) / per-flyway filter (8 flyways) / per-engine-class filter (8 §33.76 classes) / per-layer toggles HALO/PIN/LBL/FLY/APT / search by callsign/type/operator/flyway/engine/airport · precedent accident family US 1549 KLGA Hudson 2009-01-15 (NTSB AAR-10-03 · dual-CFM56-5B4/P stall · Canada Goose ~3.6 kg @ 2818ft AGL · canonical §33.76 large-bird breach cert-amdt-26 driver) · Ryanair FR4102 LIRA Ciampino 2008-11-10 (ANSV · starling flock final approach dual CFM56 power-loss hard landing) · Eastern 375 KBOS Logan 1960-10-04 (Lockheed Electra L188 starling-flock through all four PT6-class T56 on T/O · seminal §33.77 fan-blade containment cert driver) · Bek Air SCAT 760 Q400 SCATL 2019-12-27 (NTSB AAR-13-01 / KAS aviation safety committee · bird ingest on T/O roll overran) · Concorde F-WTSS DGAC 1976 (flock during cert test single-engine surge · §33.76 amdt-23 cert driver) · KLM 738 Turkish 1951 EHAM 2009 (NLD AAR adjacent strike) · BIRDX entry registered in Layers Safety & Traffic category after TIBA, ft-birdx persisted preference. Refs: 14 CFR §33.76 §33.77 §25.571 §25.775 §139.337 / EASA CS-E 800 / CS-25 App.D / AC 33.76-1 §6 medium-large bird test / AC 150/5200-32B Wildlife Hazard Mgmt / AC 150/5200-33C Attractants / AC 91-44 §3 landing-lights / FAA Wildlife Strike Database 2024 Report / ICAO Annex 14 Vol I §9.4 / Doc 9137 Pt III Wildlife Ctrl Manual / IBIS Bird-Strike Info Sys / NTSB AAR-10-03 US 1549 / NTSB AAR-13-01 Bek Air Q400 / AAIB FR4102 LIRA Ciampino 2008 / CFM Type Cert E26NE/E00088EN / GE Type Cert E00078EN/E00094EN / PW Type Cert E00087EN / RR EASA E.111 / USFWS NAWMP 2018 / Wetlands Intl Boere 2007 Waterbird Pop / AEWA / EAAFP Partnership 2024 / CMS CAF Action Plan 2008.', showBirdx, ()=>{ const nv=!showBirdx; setShowBirdx(nv); lsSet('ft-birdx', nv) }],
                ...
                ['EXTLT · Exterior-Lighting / Anti-Collision Conspicuity & Per-Phase Cockpit Light-Plan Compliance Monitor · per-airframe live evaluator of the certificated exterior-lighting subsystem state — red rotating beacon, white anti-collision strobes, position (nav) lights red/green/white, landing lights, taxi lights, runway-turnoff lights, wing inspection/scan lights, and logo lights — scoring whether each aircraft lighting CONFIGURATION matches its PHASE-OF-FLIGHT requirement per the canonical lights-on-for-taxi all-on-for-takeoff gear-lights-off-in-cruise pre-departure flow taught in every Type-Rating syllabus · structurally distinct from STALL/VMC/DEEPSTL (aerodynamic / control margin), TCAS/CPA/DAA-WC (collision-avoidance LOGIC), ADSB-INT/ACASX (surveillance broadcast quality), LASER (ground-emitter cockpit-irradiance), SUN-GLARE/BLKHOL (illusions / low-luminance scenes), NIGHT-VIS display luminance (cockpit interior luminance), CONFLICT/AIRPROX (encounter geometry), FORMATION (proximity / wing-station geometry), SQUAWK (Mode-A code assignment), STBR/RWSL (pavement-light infrastructure) — EXTLT is uniquely the AIRBORNE-PLATFORM conspicuity-emitter subsystem audit, the ON/OFF/STEADY state of every certified exterior lamp and whether the configuration is legal AND appropriate for current phase-of-flight, day/night, IMC, and proximity to controlled airspace per 14 CFR §91.205(c)(2)(3) Required night equipment + §91.209 Aircraft lights (a)(1) sunset → sunrise position lights required (a)(2) at airport surface position lights illuminated (b) anti-collision system in operation (b)(2) PIC may turn off if adverse to safety + §25.1383-1401 Powered-lighting systems Part 25 §25.1389 Position-light distribution & intensities §25.1391 Minimum intensities in horizontal plane §25.1393 Minimum intensities in vertical planes §25.1395 Maximum intensities in overlapping beams §25.1397 Color specifications §25.1401 Anti-collision light system (≥40 effective candelas white / ≥120 red 40-100 flashes/min ≥75° above-below horizontal) / EASA CS-25.1383-1401 mirror / ICAO Annex 6 Pt I §6.10 Aircraft lights operating procedures 6.10.1 anti-collision lights from start of engines 6.10.2 navigation lights night ops 6.10.3 anti-collision lights moving on movement area / ICAO Annex 2 §3.2.4 / Annex 8 Pt II §5 / Doc 4444 PANS-ATM §7.5.1.2 Standard taxi / Doc 9870 Manual on Prevention of Runway Incursions §6.3 Lights-On programme / FAA AC 91-73B §6 EXTERIOR LIGHTING REQUIREMENTS / InFO 10003 Use of Exterior Lighting to Enhance Aircraft Visibility During Taxi / SAFO 17005 Recommended Use of Aircraft Conspicuity Lights / AIM 4-3-23 Use of Aircraft Lights / Order JO 7110.65 §3-7-2 Anti-collision light operation / FAA-H-8083-25C Ch.7 Aircraft Systems - Lighting / SAE ARP 5029 Position-Light Performance / SAE AS 8037 Anti-Collision Lights minimum spec / MIL-L-6503 / ARINC 700/703 Lighting bus / Boeing AMM Ch.33 Lights every type / Airbus AMM Ch.33 / Embraer AMM Ch.33 / Bombardier AMM Ch.33 / ATR AMM Ch.33 / Boeing FCOM Vol 1 §16 Lights system description / Airbus FCOM DSC-33 / Boeing FCOM PI/QRH Pre-flight/Taxi/Takeoff LIGHTS flow line items / Airbus FCTM PRO-NOR-SOP-09 Departure Lights ON before pushback / Airbus FCTM PRO-NOR-SOP-13 Take-Off STROBES ON line-up callout · 8-aircraft-class fleet catalogue (HVY-T/HVY-Q/WB-M/NB/RGN-J/RGN-T/BIZ/LIGHT) with per-class certified lamp inventory (landing type HID/TUNGSTEN/LED + landing wattage + landing count + taxi count + RTO count + wing-scan/logo presence + switch-arch single/multi/logic) per Boeing 777/787/A350 AMM 33-40 / Boeing 747-400/-8 + A380 AMM 33-42 / Boeing 767 + A300/A310 AMM 33-41 / Boeing 737NG/MAX + A320 AMM 33-44 / E170/190 + CRJ700/900 AMM 33-44 / ATR42/72 + Q400 AMM 33-42 / Gulfstream G650 + Bombardier Global AMM 33-40 / PC-12 + Cessna 25B POH §7 · 10-lamp binary ON/OFF/PULSE state inventory: BEACON (red rotating anti-collision engine running) / STROBE (white anti-collision movement/line-up/fly) / NAV-L (red position light left wing) / NAV-R (green position light right wing) / NAV-T (white position light tail) / LANDING (sealed-beam/HID landing gear extension/approach) / TAXI (diffuse nose-gear/wing root) / RTO (runway-turnoff wing-root sideways) / WING (wing inspection/scan ice/general view) / LOGO (vertical-fin logo light) · 9-phase-of-flight classifier with per-phase REQUIRED/RECOMMENDED/OPTIONAL/OFF per AC 91-73B + Boeing/Airbus FCOM SOP + ICAO Annex 6 §6.10: PARKED All OFF / GATE-ENG BEACON required start-of-engines / PUSH-TAXI BEACON+NAV+TAXI+LOGO night (RTO optional STROBE off courtesy) / LINE-UP All ground lights ON entering runway (BEACON+NAV+STROBE+LANDING+TAXI+RTO) / TKO-ROLL Full TAKEOFF config Logo OFF interference w/ peripheral vision / CLIMB-LOW <10000 ft Below-10K conspicuity per FAA InFO 10003 / CRUISE BEACON+NAV+STROBE landing OFF HID arc-life preservation / APPR-DESC Below 10K STROBE+NAV+LANDING+LOGO / LANDING Full landing config (all on except TAXI/RTO until clear of runway then TAXI+RTO+LOGO) · 7 risk drivers (max·0.66 + mean·0.34 × phase-weight × ADV-MUL): CFG Per-phase MISSING-required lamps (count × 18) / COURTESY Per-phase TOO-MANY illegal lamps (count × 10) / NIGHT Night/IMC penalty when NAV+STROBE off in dark / CONSPIC InFO 10003 Below-10K conspicuity gap / SLD landing-light icing penalty (lamp lens icing) / LOGO-INV Logo light ON during TKO-ROLL or CLIMB-LOW cited interference w/ peripheral / ground crew dazzle on rotation / BURN Synthetic lamp-burnout proxy (HID arc-life cycle) · 5 hard escalators: BEACON OFF during engine-running ground ≥ 92 (ICAO Annex 6 §6.10.1 HARD reg violation) / NAV OFF during night ops ≥ 88 (14 CFR §91.209(a)(1) HARD reg violation) / STROBE OFF during TKO-ROLL/LINE-UP ≥ 78 (FAA SAFO 17005 InFO 10003 conspicuity gap) / LANDING OFF on TKO-ROLL/LANDING + night ≥ 74 / LOGO ON during TKO-ROLL ≥ 55 (Boeing FCOM PI §16 logo off for takeoff) · 6 tiers VIOLATION ≥85 rose Hard FAR/Annex-6 violation / UNSAFE ≥65 rose-pink Conspicuity gap immediate action / SUBOPT ≥45 amber Improvement opportunity brief crew / COMPLIANT ≥22 sky Within phase-typical config / OPTIMAL <22 emerald InFO 10003 fully met / OFF slate Parked outside phase envelope · MapLibre overlay 4-layer (tier-coloured halo ring 7-19 px score-sized + VIOLATION/UNSAFE rose pins worst-tier 2 + beam-cone vector forward from aircraft length proportional to LANDING lamp power kW × score showing conspicuity reach + per-airframe label cs · class · score · lamps-on count) · Side panel 6-tier counter strip click-to-filter ALL + 5-cell summary μ-SCORE/VIO-cnt/UNSAFE-cnt/NIGHT-cnt/Σ-LAMPS-ON + 4 sliders ADV-MUL 50-200% / NIGHT-AMP 50-200% / SLD-MUL 0-200% / BURN-MUL 0-200% + NIGHT-FORCE/IMC-FORCE/SLD-FORCE checkboxes + 8-class chip filter + 9-phase chip filter + HALO/PIN/BEAM/LBL toggles + cs/type/lamp/phase search + AIRCRAFT/CLASSES/LAMPS/PROFILE 4-tab switcher · AIRCRAFT tab tier-worst-first row stack (cs + type + class-pill + phase-pill + tier-pill + NIGHT/IMC ground-flags + 10-lamp ON/OFF strip with tier-color when wrong + tier-coloured score bar + 7-driver chips CFG/COURTESY/NIGHT/CONSPIC/SLD/LOGO-INV/BURN + tier-coloured advice line citing reg + Boeing/Airbus FCOM) · CLASSES tab per-class aggregate row (class-pill + label + count + lamp-power table + 4-cell summary μ-SCORE/VIO/SUB/lamps-ON-rate + per-class cert-citation italic note) · LAMPS tab per-lamp count strip (each of 10 lamps shown with fleet ON/OFF rate + per-lamp phase legal-mode summary + mini bar chart of ON-rate by phase per lamp vs AC 91-73B expected target) · PROFILE tab full SVG lighting-profile timeline (Y-axis 10 lamp rows / X-axis 9 phase columns PARKED→LANDING / Cell colour REQUIRED green RECOMMENDED sky OPTIONAL slate OFF forbidden rose / picked-aircraft overlay highlighted with actual state filled/empty in each phase column / Reference Boeing FCOM PI §16 / Airbus FCTM PRO-NOR-SOP-09 / FAA AC 91-73B §6 table embedded in figure) · EXTLT entry registered in Layers Safety & Traffic category after MACH-TUCK, ft-extlt persisted preference. Refs: 14 CFR §91.205 §91.209 §25.1383-1401 / EASA CS-25.1383-1401 / ICAO Annex 6 Pt I §6.10 / Annex 8 Pt II §5 / Doc 9870 §6.3 / Doc 4444 PANS-ATM §7.5.1.2 / FAA AC 91-73B §6 / InFO 10003 / SAFO 17005 / AIM 4-3-23 / Order JO 7110.65 §3-7-2 / FAA-H-8083-25C Ch.7 / SAE ARP 5029 / SAE AS 8037 / RTCA DO-160G §22 / MIL-L-6503 / ARINC 700/703 / Boeing AMM Ch.33 / Airbus AMM Ch.33 / Boeing FCOM Vol 1 §16 / Airbus FCOM DSC-33 / Boeing FCOM PI/QRH LIGHTS flow / Airbus FCTM PRO-NOR-SOP-09 Departure / Airbus FCTM PRO-NOR-SOP-13 Take-Off / UK CAA CAP 715 §2 / EASA Easy Access Rules Part-NCO NCO.OP.180 / TC AIM AIR §2.7 / EASA SIB 2017-13 Conspicuity of Aircraft on Ground / NTSB AAR-91-08 LAX1493 USAir 1493 / SkyWest 5569 KLAX 1991-02-01 / NTSB AAR-08-01 Comair 5191 KLEX 2006-08-27 / NTSB DCA24MA063 Alaska 1282 / JTSB AI-2024-1 JL516/JA722A KHND 2024-01-02 / ASN WO #19980402-1 KLM 4805 LV-LTN Tenerife 1977', showExtlt, ()=>{ const nv=!showExtlt; setShowExtlt(nv); lsSet('ft-extlt', nv) }],
                ...
                ['UERF · Uncontained Engine Rotor Failure & Disk-Burst Debris-Trajectory / 1-in-20-Event Containment Monitor · per-airframe live evaluator of UNCONTAINED-ENGINE-FAILURE exposure for every airborne aircraft, scoring whether a rotor-burst hazard (high-energy fragment exiting the nacelle through the certificated containment shield) would pose a structural / systems / occupant threat at the current phase, thrust setting and pressurization state per 14 CFR §33.94 Blade containment & rotor unbalance test (single-blade-out at red-line+5% overspeed) / §33.27 Turbine compressor fan rotor integrity (122-130% red-line ultimate disk-burst margin) / §33.19 Durability fragment energy retention / §33.74 Continued rotation post-failure / §25.901(c) Powerplant installation safety / §25.903(d)(1) Engine isolation against UER protection of essential systems and second engine (the canonical "1-in-20 events shall not jeopardise continued safe flight and landing" risk threshold) / §25.905(d) Propeller-debris hazard / EASA CS-25.903(d)(1) / CS-E 810 Containment / CS-E 840 Rotor integrity / FAA AC 20-128A Design Considerations for Minimizing Hazards Caused by Uncontained Turbine Engine and APU Rotor Failure (foundational reference defining 1/3-disk + intermediate + alternator fragment classes with ±5° / ±15° spread cones, minor/major/hazardous/catastrophic debris-zone categorisation, 1-in-20-events probabilistic acceptance framework used by all OEMs for systems isolation) / FAA AC 25-19A Continued airworthiness of engines / SAE ARP4761 §A.3 UER risk-class allocation / SAE AIR-4003 Debris-zone classification / SAE AS-682 Fan-blade-out cert / MIL-STD-1797B §4.2 Asymmetric-thrust / ICAO Annex 8 Pt IIIA §1.2 Engine cert / Doc 9760 Vol II Pt VI · foundational UER accident record ATSB AO-2010-089 QF32 2010-11-04 A380 VH-OQA Trent 900 #2 IPC stage-2 disk uncontained burst departing WSSS 50+ debris hits 21/22 hyd circuits + electrical buses + fuel tanks compromised 4-hr controllability fight (defining 21st-century UER event) / NTSB AAR-90-06 UAL232 1989-07-19 DC-10 N1819U GE CF6-6 #2 fan disk fracture FL370 over Sioux City all 3 hyd circuits severed throttle-only control 184/296 survived KSUX / NTSB AAR-19-03 SWA1380 2018-04-17 737-700 N772SW CFM56-7B #1 fan blade FBO cruise FL320 fan-case penetrated cabin window 1 fatality / NTSB DCA21IA095 UAL328 2021-02-20 777-200 N772UA PW4077 #2 fan blade nacelle inlet/fan-cowl departure debris field Broomfield CO / NTSB AAR-18-01 AA383 2016-10-28 767-300 N345AN GE CF6-80C2 #2 HPT stage-2 disk burst during RTO KORD fuel-fed fire hull loss / NTSB AAR-98-01 DL1288 1996-07-06 MD-88 N927DA P&W JT8D-219 #1 stage-1 fan-hub fracture KPNS 2 fatalities triggered FAA AD on JT8D-219 fans · 16-class rotor/engine catalogue (CFM LEAP-1A/-1B fboMrgn 7-8% diskMrgn 23-24% / CFM56-7B 11/28% SWA1380 precedent / CFM56-5B-V2500 14/30% / GE CF6 9/22% UAL232+AA383 / GE GE90-115B 13/27% / RR Trent-XWB-900 12/26% QF32 / RR Trent-1000-7000 11/25% AD-chain 2017-19 / PW JT8D-219 9/22% DL1288 / PW1100G GTF 9/25% / GE CF34 16/30%) · 8 drivers each 0-100 THR thrust setting %MCT (UER probability scales with HPT EGT) / CYC rotor-cycle exposure (disk LCF life proxy per §33.27) / FBO fan-blade-out containment margin gap (§33.94) / DSK disk-burst margin gap (§33.27 122% ultimate) / ZONE debris-zone occupancy of essential systems per AC 20-128A 1/3-disk fragment cone / PRES cabin Δp at FL (penetration depressurisation risk per SWA1380 window) / MULT multi-system loss potential per QF32/UAL232 pattern / ISO §25.903(d)(1) engine-isolation residual margin · composite max·0.30·DSK + 0.18·CYC + 0.15·ZONE + 0.10·FBO + 0.10·MULT + 0.08·ISO + 0.05·PRES + 0.04·THR × phase-weight × ADV-MUL · hard escalators disk-gap<8% in TKO/GA/INI-CLB score-min 92 (defer T/O per §33.27) / FBO<5% + CYC>80% min 84 (§33.94 risk per SWA1380) / CRZ + disk<12% + multi-system>55% min 78 (QF32/UAL232 isolation-loss pattern) / fleet-density UER history≥50 + CYC>85 + TKO/GA min 72 / cabin Δp>8psi + zone>55% in CRZ min 65 (SWA1380 precedent) / iso-margin<35% in TKO/GA min 58 · 6 tiers CATAS-IMM 1-in-10⁹ rose / HAZARD 1-in-10⁷ rose-pink / MAJOR 1-in-10⁵ amber / WATCH transient sky / NOMINAL design envelope emerald / OFF non-thrust phase slate · MapLibre debris-zone cone polygons (1/3-disk ±5° forward fragment + ±2.5° lateral outboard cones colour-coded by tier) + tier-coloured halo+pin+aircraft labels with eng/DSK%/CYC% · AIRCRAFT/ENGINES/DEBRIS-ZN/DRIVERS tabs · 4 sliders ADV-MUL / DSK-FLOOR 8-30% / CYC-CEIL 60-100% / thrust-only gate · 8-phase chip filter · 11-engine chip filter · HALO/PIN/LBL/CONE toggles · structurally distinct from CSURGE (open-loop HPC surge margin — UPSTREAM aerodynamic stability), VIB/FBO (vibration FFT detection POST-imbalance), HOTSEC (thermal-life consumption), EGT/EGTM (thermal trend), OIL (lubrication health), RELIGHT (restart envelope), CARGOFS (consequence-side fire suppression), HYDRAULIC redundancy (consequence-side of UER damage), ELEC (consequence-side electrical bus), MEL (paperwork) — UERF is uniquely the FORWARD-LOOKING structural-integrity probability scorer for an UNCONTAINED rotor fragment exiting nacelle containment per AC 20-128A debris zone cones, the margin that determines whether the next high-thrust event sits on the right side of the "1-in-20-events shall not jeopardise continued safe flight" threshold governing transport-category cert · UERF entry registered in Layers Safety & Traffic category after CSURGE, ft-uerf persisted preference', showUerf, ()=>{ const nv=!showUerf; setShowUerf(nv); lsSet('ft-uerf', nv) }],
                ...
                ['BTV · Brake-To-Vacate / ROPS Runway-Overrun-Prevention-System Exit-Selection Monitor · per-airframe live evaluator of the certified Airbus BTV (Brake-To-Vacate) system layered on top of ROPS, scoring every aircraft in late-final / flare / landing-rollout phase at one of 28 catalogued aerodromes for which a BTV-style exit-vacate prediction is physically valid: given the projected touchdown-zone (TDZ), required rollout-distance D_req(V_TD, V_exit, μ_eff, slope, wind) and the runway rapid-exit-taxiway (RET) inventory (entry chord, exit speed envelope, throat distance from threshold), does the crew-selected (or auto-recommended) exit match the achievable autobrake demand without overrunning the runway end and without missing the chosen exit at speed · structurally distinct from ROW/ROP (overrun WARNING reactive vs LDA), HIRO/RET (rapid-exit infrastructure geometry inventory only), BRAKE-TEMP (carbon-disc thermal margin / Tau_brake fuse-plug melt), OLD (Operational Landing Distance at-the-fence comparator), EMAS/RESA (engineered arrestor bed past runway end, last-line of defence), RAAS (aural distance-remaining callouts via SmartRunway/SmartLanding), HYDROPLANE (dynamic aquaplaning V_p = 9√P_t), FOD (debris detection regime), STBR/RWSL (in-pavement enforcement) — BTV is uniquely the PROACTIVE EXIT-CHOICE OPTIMISER: given V_TD, μ, slope, wind, mass and exit catalogue, choose the latest RET i* such that D_req ≤ x_i − x_TDZ and required a_dec is within the OAUW (Operational Autobrake Upper Limit) envelope · canonical precedent Air France 358 (F-GLZQ A340-313X) CYYZ 2005-08-02: YYZ arrival 24L, ALD 2743m, runway wet w/ thunderstorm and ~10kt tailwind shift through flare, floated long, touched down ~1280m past threshold leaving ~1463m, reverse deployment delayed ~12.8s, departed end at ~80kt, traversed 240m RESA, came to rest in Etobicoke Creek ravine post-impact fuel-fed fire, 309 souls evacuated 12 minor, hull loss (TSB A05H0002 explicitly cited absence of exit-selection / wet-runway-μ aware decision support as counterfactual — BTV-MAX A380 EIS 2009+ was designed precisely to surface this decision pre-touchdown) · 10 airframe classes A380 BTV-MAX OAUW 0.32g / A350 BTV OAUW 0.31g / A330neo BTV OAUW 0.30g / A330 ROPS-retrofit OAUW 0.30g / A320neo ROW/ROP OAUW 0.28g / B777 Honeywell SmartRunway OAUW 0.30g / B787 SmartLanding OAUW 0.30g / B737 SmartLanding OAUW 0.28g / RGN-J OAUW 0.26g / OTHER OAUW 0.25g · 28-runway global BTV-relevant catalogue spanning CYYZ 24L (AF358 precedent) / KJFK 04R+13L / KLAX 25L+24R / KORD 10C+28R / KATL 26L / KSFO 28L+28R (Asiana 214 vicinity over-bay EMAS) / KSEA 16R / KBOS 33L (EMAS-end) / EGLL 27L+09R / LFPG 26R+08L / EDDF 25C / EHAM 18R Polderbaan / EDDM 26L / OMDB 12L+30R Emirates A380 base / OTHH 16L Qatar A380 base / WSSS 02L SQ A380 base / VHHH 07L Cathay A350 base / RJTT 34L JL A350 base / YSSY 16R QF A380 base / LTFJ 06 SAW Pegasus 2193 2020 overrun precedent · each runway carries threshold position, LDA, slope, surface PCC/AC/GR, RESA depth, and full RET inventory with throat distance from threshold and per-RET maximum chord-exit speed (60kt HSET / 30kt 90°-right-angle per ICAO Annex 14 §3.9.6 / Doc 9157 Pt 2 §1.10.2) · 6-driver scorer EXIT (gap between required a_dec and available a_dec at chosen RET) / MU (surface friction from RCAM code DRY→ICE 0.40→0.05) / WIND (landing-direction component, +tailwind = overrun-risk Vap penalty 0.7kt/kt) / SLOPE (downhill −% reduces braking effectiveness) / ALIGN (heading vs runway-bearing error, large = unstable approach) / MASS (landing mass relative to class median) · composite max·0.65 + mean·0.35 × ADV-MUL with hard escalators OVERRUN ≥ 92 / MISS-EXIT (forced runway-end) ≥ 72 / ALIGN-busted in FLARE/ROLLOUT ≥ 60 · 6 tiers OVERRUN (a_req > a_avail at runway-end go-around rose) / MISS-EXIT (no usable RET full-stop rollout rose-pink) / TIGHT (feasible but >0.85 of envelope amber) / NOMINAL (exit feasible MED autobrake emerald) / EARLY-EXIT (LOW autobrake comfortable sky) / N/A (out of scope slate) · MapLibre overlay runway centrelines dashed sky + per-RET chevrons perpendicular to runway coloured by best-exit selection tier or slate-grey reference-only + tier-coloured aircraft halos 8-21px + tier-coloured pins with cs › IATA RWY exit-name labels + OVERRUN flag · AIRCRAFT/RUNWAYS/PHYSICS tab switcher · AIRCRAFT tier-worst-first row stack with cs+type+class-label-with-BTV-★+tier-pill + rwy/phase/exit-name/μ-label/a-avl/wind 6-cell + tier-coloured score bar + 6-driver chip strip EXIT/MU/WIND/SLOP/ALIG/MASS + tier-coloured advice line citing FCOM PRO-NOR-SOP-19 / TSB A05H0002 click-to-fly cinematic zoom-13 runway-level · RUNWAYS table sortable by LDA with ICAO+IATA/LDA/SLOPE-%/RETs-count/RESA-m/SURFACE columns + RCAM friction-code reference table 6-DRY 0.40 down to 1-ICE 0.05 · PHYSICS tab renders AF358 precedent + BTV equation D_req = (V_TD² − V_exit²) / (2·a_dec_eff) + a_dec_eff decomposition μ·g + a_rev + slope·g − wind_assist + per-RET a_req_i formula + i* selection rule + 6-driver glossary + autobrake OAUW envelope table A380→B737 OAUW/MAX/MED/LOW + DISTINCT-FROM ROW-ROP/OLD/HIRO-RET/BRAKE-TEMP/EMAS/RAAS/HYDROPLANE/FOD/STBR-RWSL + regulatory framework Airbus FCOM PRO-NOR-SOP-19 / DSC-32-30-30-20 / FAST 51 (Jan 2013) BTV system description / FAST 54 (Jul 2014) ROPS expansion to A320 family / AAWG ROPS-BTV White Paper / RTCA DO-367 ASA MOPS / DO-358 DO-329 overrun awareness / FAA AC 25-32 landing performance / AC 91-79B mitigating runway-overrun risk / AC 25.1322-1 alerting / SAFO 06012 19001 in-time landing performance assessment / EASA CS-25.109 .125 .1322 / EASA TCDS A.110 A380 / ICAO Annex 14 Vol I §3.10 §3.9.2 RET geometry / Doc 9157 Pt 2 §1.10 §6 Aerodrome Design Manual RET / Doc 9981 PANS-AGA Pt II §4 / Annex 6 Pt I §4.3.7 at-the-fence / TSB Canada A05H0002 Air France 358 Final Report / NTSB AAR-08-02 Comair 5191 related runway-id / NTSB DCA17IA148 Air Canada 759 SFO TXY-C alignment related / AAIB EW/C2020/05/02 Pegasus 2193 SAW overrun BTV-relevant / ATSB AO-2010-082 Jetstar A320 ZBAA hard-rollout BTV-relevant · 6-tier counter strip + 4-cell summary μ-SCORE/IN-SCOPE/RWYS/WORST-cs + ADV-MUL 50-200% slider + HALO/PIN/RET/CL toggles + filter input by cs/type/rwy', showBtv, ()=>{ const nv=!showBtv; setShowBtv(nv); lsSet('ft-btv', nv) }],
                ['PNR · Point-of-No-Return / Last Safe Return Window Monitor · per-airframe live evaluator scoring every cruising aircraft\'s proximity to its computed PNR — the latest geographic point along the route at which the aircraft can still return to a usable diversion airport (origin / en-route alternate / ETOPS-suitable) with required fuel reserves intact, OR continue to destination, given current ground-speed, head/tail-wind component, fuel-on-board endurance estimate and field availability · structurally distinct from ETP/Critical-Point (geographic equal-TIME point return-vs-continue, time-symmetric not fuel-symmetric), ETOPS Monitor (Diversion-Time threshold cert compliance 60/120/180/240/330), DRFTDN (single-engine descent profile to MORA crossing), ALTN-Suit (alternate-airport suitability scoring wx/RVR/RWY), REDISPATCH RCF (FAR 121.631(c) re-clearance fuel-saving Sec.B construct), REACH-Atlas (single-engine flame-out glide reach), RESERVE fuel (basic 14 CFR 121.639 IFR floor 45min+alt+holding), FUEL-TANKERING (economic carry-extra-fuel decision), GLD glide reach, PRD payload-range pre-dispatch trade-off — PNR is uniquely the LAST-WINDOW FUEL-DECISION concept, the moment past which you can no longer turn back, once across PNR the aircraft is COMMITTED to destination or any forward-only alternate with the return option fuel-extinguished · canonical precedent Air Transat 236 (C-GITS A330) LPLA 2001-08-24: YYZ→LIS sector, fuel leak from #2 engine starboard detected at 04°27W (already past computed PNR at ~37°W for return to CYYZ/CYHZ), leak rate ~1.2T/min transitioned CLEAR→IMMINENT→COMMITTED in 24 minutes, Captain Robert Piché executed 75 NM dead-stick glide at FL340 to total flame-out and landed both engines out on LPLA 33 Lajes/Azores, 18-min powerless glide, 306 souls survived, 2 minor injuries on evac (TSB A01F0093) · 9 airframe classes WB-ULR (B77L/B77W/B789/B78X/A359/A35K/A388 endurance 18.5h ETOPS-330) / WB-LR (B772/A332/A333/A338/A339/A340/MD11 14.5h ETOPS-240) / WB-MED (B762/B763/B764/A300/A310/B744/B748 11.5h ETOPS-180) / NB-LR (B752/A21N/A321/BCS3 7.5h ETOPS-180) / NB (B737/A320 5.5h ETOPS-120) / RGN-J / RGN-T / BIZ / LIGHT with per-class cruise TAS / cruise fuel-flow kg-hr / max endurance hr / final-reserve min / ETOPS approval min / L-D glide ratio · 20-region oceanic/remote/hostile-terrain catalogue NAT (N-Atlantic OTS) / PACOTS / POLAR (lat>70°) / AUSEP (Australasia SW-Pac) / BOB (Bay of Bengal) / SAH (Sahara/N-Africa) / SATL (S-Atlantic) / EQPAC (equatorial S-Pacific) / RUS (Russian Far East) / TSIB (Trans-Siberian) / GRN (Greenland/Davis Strait) / CASIA (Central Asia) / SCS (S-China Sea) / CARIB (Caribbean) / BER (Bermuda triangle) / HAW (Hawaii mid-Pacific) / EAST (Easter Is./SE-Pacific) / CHRN (Cape Horn/Drake Passage) / MALDV (Maldives/equatorial-IO) / MIO (mid-Indian-Ocean) — each region carries 2-4 catalogued return fields with CAT-IIIB/CAT-II/CAT-I/NPA certification, runway length, and per-region exposure factor 0.72-1.00 (Caribbean lowest, Easter-Island/CapeHorn/POLAR highest) · canonical PNR equation t_pnr = E × GS_ret / (GS_cont + GS_ret) with E = (FOB × endurance_max) − reserve_min, GS_cont = TAS + wind_along, GS_ret = TAS − wind_along, D_pnr = t_pnr × GS_cont, COMMITTED ⇔ D_return / GS_ret > E (return field unreachable within endurance) · 7-driver scorer FOB (fuel-on-board fraction drives endurance hours remaining vs final-reserve floor) / WIND (head/tail-wind component asymmetric impact on continue vs return GS) / FIELD (return-field cert quality CAT-IIIB→CAT-II→CAT-I→NPA) / ETOPS (required diversion-time vs cert envelope) / RESERVE (final-reserve fuel margin 14 CFR 121.639/.645) / TERRAIN (region exposure factor) / WEATHER (destination/alternate wx penalty) · composite max·0.6 + mean·0.4 × ADV-MUL with hard escalators (pastPNR=92, <15min=78, <60min+remote+lowFOB=70) · 6 tiers COMMITTED (past PNR, return field unreachable, rose-500) / IMMINENT (<15min to PNR, rose-400) / MONITOR (15-60min, amber) / CLEAR (>60min, sky) / PRE-OCEAN (climb/TMA/ground before remote, emerald) / LAND-AVL (land options abundant, slate) · MapLibre overlay 20 region rectangular polygons tinted by exposure (hi rose/md amber/lo sky) + return-field cyan dots with IATA labels + aircraft halos sized 7-21px by tier severity + 60-worst pin labels formatted "callsign › field t-Xm" for in-window or "callsign COMMITTED Xm past" + aircraft→return-field dashed link lines tier-coloured · Side panel 6-tier counter strip (COMMITTED/IMMINENT/MONITOR/CLEAR/PRE-OC/LANDAV) + 4-cell summary μ-SCORE/IN-SCOPE/REGIONS/WORST + ADV-MUL 50-200% slider + HALO/PIN/REG/LINK toggle chips + cs/type/region filter search + AIRCRAFT/REGIONS/PHYSICS tab switcher · AIRCRAFT tab tier-worst-first row stack with cs+type+class-pill+tier-pill + region/phase/return-field+distNM/FOB%/signed-time-to-PNR cells + tier-coloured score bar + 7-driver chips + tier-coloured advice line citing Doc 9976 §3.3.4 / AC 91-70B §6.5 with continue/divert recommendation click-to-fly · REGIONS tab exposure-sorted region row table with ID/exposure%/field-count/best-CAT/bbox-lat/bbox-lng + CAT-IIIB/II/I/NPA decision-height/RVR legend · PHYSICS tab Air Transat 236 precedent narrative + PNR equation block (t_pnr = E·GS_ret/(GS_cont+GS_ret), D_pnr, COMMITTED criterion) + 7 driver definitions + distinct-from comparison against ETP/ETOPS/DRFTDN/ALTN/REDISPATCH/REACH/RESERVE/GLD/PRD + regulatory framework ICAO Doc 4444 PANS-ATM App.2 / Doc 7030 RAC / Annex 6 Pt I §4.3.6 / Doc 9976 §3.3 / Doc 10085 EDTO / Doc 8168 PANS-OPS Vol I Pt VIII / FAA AC 120-42B / AC 91-70B / Order 8900.1 Vol 4 Ch 4 §9 / 14 CFR 121.625/.631(c)/.639/.645/.646 / EASA Reg 965/2012 CAT.OP.MPA.140 / AMC1 CAT.OP.MPA.181 / IATA Fuel Conservation Guide §4 PDP + region-exposure ranked list. PNR entry registered in Layers Safety category after Fire-loop, ft-pnr persisted preference.', showPnr, ()=>{ const nv=!showPnr; setShowPnr(nv); lsSet('ft-pnr', nv) }],
                ['FOD · Foreign-Object-Debris Runway-Contamination & Engine-Ingestion Risk Monitor · per-airframe live evaluator of FOD (Foreign-Object Damage/Debris) exposure for every aircraft currently in a runway-rollout phase (T/O roll, rotation, touchdown, landing roll, taxi-on-active) at one of 22 catalogued aerodromes — scoring whether the runway sweep cycle, FOD-detection equipage, ops density and per-airframe ingestion-cert envelope leave the aircraft vulnerable to a debris encounter on the pavement that has not yet been detected and removed · structurally distinct from BIRDX (bird/wildlife flock ingestion, organic, §33.76 cert family), HAIL (frozen-hydrometeor ingestion, §33.78, in-flight), HOLDOVER (anti-ice fluid HOT countdown chemical film integrity), HYDROPLANE (wet-runway dynamic aquaplaning tire vs water), RCAM/TALPA (friction-code reporting), ASDE-X (surface-movement radar coverage), HOTSPOT (cartographic incursion hot-spot registry), RAAS (aural runway-identity callouts), STBR (illuminated stop-bar enforcement), PCN (pavement load-classification number) — FOD is uniquely the INORGANIC DEBRIS detection-and-sweep regime: loose metal, concrete spalls, rubber chunks, lost hardware, vehicle litter, snow-plow ice, tire fragments, fasteners, dropped cargo, runway-edge gravel that can be ingested by jet engines, sliced by tires, or cause hydraulic-line strike on rotation · canonical precedent Air France 4590 (Concorde F-BTSC) LFPG 2000-07-25: a 43cm × 3.4cm titanium wear-strip lost by preceding Continental DC-10 (N13067) during its takeoff roll on runway 26R was struck by the Concorde\'s #5 tire at V1−14kts, sending a 4.5kg rubber chunk into the underwing fuel tank, ignition, total hull loss 109 + 4 fatal (BEA F-SC000725) · 22-aerodrome global FOD-tracked catalogue spanning LFPG (Concorde precedent) / EGLL Heathrow (Tarsier mmWave 24/7 since 2007 world-first) / KORD O\'Hare (8-runway parallel ops + east-side construction) / KATL Atlanta (Trex MS-imaging 2018) / KLAX Los Angeles (Trex 24L/06R 2019 + midfield construction) / KJFK New York (visual sweep + Terminal-1 construction) / KDFW Dallas-Fort-Worth (7-runway grid) / KMIA Miami (heavy LATAM cargo + tropical-storm debris) / KIAH Houston-Bush / KBOS Boston-Logan (Tarsier 04R/22L since 2010 harbour-salt PCC spall) / KDEN Denver / KSFO San Francisco (Asiana 214 precedent + Harvey-Milk Terminal construction) / KSEA Seattle / EHAM Amsterdam (X-Sight FODetect Polderbaan since 2014) / EDDF Frankfurt (Tarsier 07C/25C since 2014 + Pier-J cargo extension) / EDDM Munich (FODetect on 08L/26R + snow-clearance cycle) / OMDB Dubai (Tarsier 24/7 12L/30R and 12R/30L since 2009) / OTHH Doha-Hamad (Tarsier from terminal-open 2014) / WSSS Singapore-Changi (Stratech iFerret vision-AI 02C/20C + T5 construction adjacent) / VHHH Hong Kong (iFerret 07L/25R after 3-rwy 2022) / RJTT Tokyo-Haneda (FODetect partial + JL516/JA722A 2024 mid-RWY collision adjacent) / YSSY Sydney (visual + harbour-bird/litter mix) · per-field FOD-detection equipage taxonomy per AC 150/5220-23: QinetiQ Tarsier (1.5min latency 99% coverage), Stratech iFerret (2.0min/97%), X-Sight FODetect (2.5min/95%), Trex/FOD-Finder (3.0min/90%), §139.327 visual sweep (120min/85%), opportunistic/uncertified (480min/50%) · 8-class airframe FOD-vulnerability catalogue with per-class inlet-area (6.30m² 4-eng WB → 0.35m² LIGHT), §33.77 medium-bird ingest-cert mass (3.65kg WB → 0.45kg LIGHT), main-gear tire psi (230 WB → 60 LIGHT — Concorde was 232psi, compression-failure on Ti strike), and ground clearance for gravel pick-up · debris-dwell physics t_dwell = max(t_detect_latency, min(t_inspect_gap, t_detect + t_gap·(1−coverage))) with t_inspect_gap = 1440/N_inspect/day and P(encounter) = 1 − exp(−λ · t_dwell · ops/min · rate_per_Mops · scope) · 7 risk drivers INSP (§139.327 daily self-inspection count compliance, floor 3/day excellent 6/day) / DETECT (installed FOD-detection equipage latency penalty Tarsier 1.5min → Visual 120min) / RATE (operations per day saturation of sweep window >2000 saturates) / VULN (airframe ingestion vulnerability inlet-area cross-section + tire-pressure puncture margin) / CONSTRUCT (active construction adjacent to RWY debris-shed risk window per AC 150/5380-5C) / PHASE (T/O-roll > landing-rollout > rotate > taxi — Concorde scenario peaks at V1−10kt phase-weight 1.18) / PAVE (incident rate per million ops, FOD-Net 2024 baseline 0.5-3.5 per Mops globally) · composite max·0.62 + mean·0.38 × phase-weight × ADV-MUL · hard escalators T/O-ROLL at NONE-detect field with active construction = 88 / T/O-ROLL of WB-HVY or WB-MED at construction-active field = 78 / LIGHT in T/O-ROLL = 55 · 6 tiers CRITICAL≥85 rose / HIGH≥65 rose-pink / ELEVATED≥45 amber / WATCH≥22 sky / NOMINAL≥8 emerald / IDLE<8 slate · MapLibre overlay tier-coloured halo rings 8-21px + tier-coloured pins + dashed link aircraft→field + per-field circular FOD-detection zones (radius scaled by ops density, fill colour tinted by debris-dwell-time severity hi/md/lo) + 22 airport markers with IATA+DETECT labels · AIRCRAFT/FIELDS/PHYSICS tab switcher · AIRCRAFT tier-worst-first row stack with cs+type+class-label+tier-pill + field-iata+phase+distNM+detect-class+CONSTR-pill + tier-coloured score bar + 7-driver chip strip · FIELDS table sortable by debris-dwell-time with ICAO/IATA/DETECT/INSP-per-day/OPS-per-day/incident-rate/DWELL-min/CONSTR-flag + detect-class taxonomy reference table + dwell-latency colour gradient · PHYSICS tab renders Concorde precedent + debris-dwell model + §33.77/§33.78 cert ingestion envelope + 7-driver glossary + regulatory framework (FAA AC 150/5210-24 / AC 150/5380-5C / 14 CFR §139.305/.307/.327 / §33.77/.78 / ICAO Annex 14 Vol I §10.2 / Doc 9137 Pt 8 / Doc 9981 PANS-ADR / EASA CS-ADR-DSN.M.625 / SAE ARP5388B / BEA F-SC000725 / ATSB AO-2017-119 / FOD-Net 2024 ~$13B annual cost) + per-detector dwell-time worked examples · 6-tier counter strip + 4-cell summary μ-SCORE/IN-SCOPE/FIELDS/WORST-cs · 2 sliders SCOPE 5-40NM / ADV-MUL 50-200% + HALO/PIN/ZONE/LINK toggles + cs/type/icao filter input · click-to-fly cinematic zoom to airframe at zoom-11 runway-level (FAA AC 150/5210-24 FOD Mgmt / AC 150/5380-5C debris hazards / AC 150/5380-6C pavement maintenance / AC 150/5200-30D winter safety / 14 CFR §139.305/.307/.327 Part 139 self-inspection / §33.77 small/medium-bird & ice ingestion cert / §33.78 hail & large-bird cert / FAA Order 5200.11 Airports FOD program / JO 7110.65 §3-1-12 runway condition / ICAO Annex 14 Vol I §10.2 movement areas / Doc 9137 Pt 8 airport services / Doc 9981 PANS-ADR Pt II Ch.5 surface inspection / EASA CS-ADR-DSN.M.625 apron/manoeuvring inspection / AMC1 ADR.OPS.B.015 FOD detection systems / IATA AHM 920 §6 ground-handling debris discipline / SAE ARP5388B engine FOD prevention / ARP1797D tire damage assessment / NTSB DCA00MA079 Concorde co-investigation / BEA F-SC000725 AF 4590 Final 2002-01 / AAIB 4/2008 BA 38 LHR / ATSB AO-2017-119 QF B738 BNE 2-eng FOD / NTSB AAR-08-02 Comair 5191 LEX / FAA SAFO 17009 contaminated runways / InFO 11008 FOD-related runway-inspection / FOD-Net Annual 2024 ~$13B industry-wide)', showFod, ()=>{ const nv=!showFod; setShowFod(nv); lsSet('ft-fod', nv) }],
                ['HZMT · Dangerous-Goods Cargo Carriage Compliance · Lithium-ion ≤30% SoC cap (ICAO Doc 9284 Add.4 post-UPS6) · IATA DGR 9.3 / 49 CFR 175.78 hazard-class segregation matrix · Class-7 RAM Transport-Index vs IAEA SSR-6 §572 cap · dry-ice 200kg compartment limit · forbidden-PAX article enforcement (49 CFR 171.16 ramp-stop) · NOTOC Notification-to-Captain DGR 9.5 delivery compliance · 40-entry UN-numbered article catalogue (UN3480/UN3481 Li-ion bulk/equip · UN3090/UN3091 Li-metal · UN1845 dry-ice · UN2912/UN2915/UN2978 RAM · UN1942/UN2014/UN3149 oxidisers · UN0027/UN0454/UN0336 explosives · UN1017/UN1005 toxic gas · UN1203/UN1170/UN1090 flam-liq · UN1381 phosphorus · UN1428 sodium · UN1789/UN1830 corrosives · UN2807 magnetised) · 14-class IATA DGR §3 portfolio · 7-driver scorer SEG/SOC/RAM/DICE/FBN-PAX/NOTOC/ER · 6 tiers UNLAWFUL≥85 rose / CRITICAL≥65 rose-pink / ELEVATED≥45 amber / WATCH≥22 sky / CLEAN<22 emerald / NOLOAD slate · phase-weighted GATE-DEP×1.30 / CRUISE×1.10 / APPROACH×1.00 · MapLibre overlay tier-coloured halo rings 7-19px + UNLAWFUL/CRITICAL rose pins + dashed link aircraft→destination + UN-article chips for top-20 worst · Side panel 6-tier counter strip + 6-cell summary μ-SCORE/DG-FLT/Σ-WH-LI/Σ-TI/Σ-DICE-kg/WORST-cs + 5 sliders ADV-MUL 50-200% / DG-EXP 0-200% / SOC-CAP 20-50% (Add.4 derate) / TI-CAP-PAX 25-100 (IAEA SSR-6 PAX-flight derate) / NOTOC-EXP 0-30% (undelivered rate) + 7-phase chip filter + 14-class chip filter + HALO/PIN/LBL/LINK/CHIP toggles + search by callsign/type/operator/UN-number · AIRCRAFT/ARTICLES/SEGMTRX/LI-CHART tab switcher · AIRCRAFT tier-worst-first row stack with cs+type+carrier-class-pill+phase-pill+tier-pill + DG-count/Σ-WH/Σ-TI/DICE-kg 4-cell + flag chips FBN-PAX/SOC-OVER/SEG-CONFLICT/NOTOC-UND/RAM/DICE-OVER/ER-CODE + tier-coloured score bar + 7-driver chips + tier-coloured advice line citing ICAO Add.4 / IATA DGR 9.3 / 49 CFR 175.78 / 49 CFR 171.16 / IAEA SSR-6 §572 click-to-fly · ARTICLES top-22 UN-roll-up by Σ-flights with class/PG/ER/PI cells + worst-tier stripe + flag chips · SEGMTRX full 14×14 IATA DGR 9.3 / 49 CFR 175.78 hazard-class segregation matrix as colour-coded grid (emerald=allowed / sky=separated / amber=holds-apart / rose=forbidden-together) + legend + UPS6 precedent narrative · LI-CHART full SVG plot of Wh-cell × SoC% scattering live Li-ion entries vs ICAO Add.4 30% SoC cap line (rose dashed) and 100 Wh Sec.IB/Sec.II split (amber dashed) + PI-965/PI-966-967/over-cap legend + Σ-PI-965/Σ-PI-966-967/Σ-OVER-cap 3-cell + methodology narrative + ICAO Doc 9284 Add.4 / IATA DGR §10.2 / PHMSA 49 CFR 173.185 / UN Manual of Tests Pt III §38.3 / FAA InFO 17013 references · Distinct from LIBAT (in-flight Halon-1301 suppression) / CARGOFS (Class-C/D/E compartment fire-suppression endurance + ETOPS DTLD) / FIRELOOP (engine/APU/cargo fire-detection loop A/B integrity) / CSFF (centre-tank flammability nitrogen-inerting) / PAXO2 (cabin emergency-descent oxygen mask) / OWL (overweight-landing fuel-jettison) / MEL (MEL deferral compliance). HZMT is uniquely the shipper-loaded manifest declaration-compliance layer answering: does the airframe legally hold the declared UN-articles, are class-conflicts absent, is the Li-ion SoC cap satisfied, has the NOTOC been delivered, are PAX-forbidden articles segregated to a CAO frame, are Class-7 TI/CSI sums within aircraft limits. NTSB AAR-98-03 ValuJet 592 / GCAA AAI 13/2010 UPS-6 N571UP / KARAIB AAB-2011-01 Asiana 991 / NTSB DCA13RA075 FedEx LX1602 / ATSB AO-2020-019 precedents. HZMT entry registered in Layers Safety category after Fire-loop, ft-hzmt persisted preference.', showHzmt, ()=>{ const nv=!showHzmt; setShowHzmt(nv); lsSet('ft-hzmt', nv) }],
                ...
                ['APBN · Approach-Ban / Approach-Continuation Rule Compliance Monitor · per-airframe live evaluator of whether each inbound flight may LAWFULLY CONTINUE past the OM/FAF/1000ft AGL gate given currently reported RVR/VIS vs the per-approach minima (CAT-I 550m · CAT-II 300m · CAT-IIIA 175m · CAT-IIIB 50m · LNAV-VNAV · LNAV · RNP-AR · CIRCLING) under the applicable regulatory regime (FAA 14 CFR §121.651(c) · EASA Part-CAT.OP.MPA.305 · UK CAP 670 · TCCA CARs 602.128/129 · CASA Part 91 MOS Ch.7.16 · CAAC CCAR-121.R5 · ICAO Annex 6 Pt I §4.4.1.3) with 7 driver scores (RVR-deficit / VIS-deficit / ceiling-vs-HAT / ground-equip-fit / OpSpec auth gap / OM-gate distance / RVR trend) aggregated to a 6-tier BAN/CRIT/MARGIN/WATCH/CLEAR/OFF risk band', showApbn, ()=>{ const nv=!showApbn; setShowApbn(nv); lsSet('ft-apbn', nv) }],
                ...
                ['CRDA · Converging Runway Display Aid & Ghost-Target Projection Monitor · per-flight live evaluator of the FAA Order JO 7110.65BB §5-9 controller decision-support tool that projects a synthetic ghost of an aircraft on one runway final-approach corridor onto the FAC of an INTERSECTING / CONVERGING / DEPENDENT-CONVERGING runway plus a per-pair stagger offset so a single approach/tower controller can apply visual or radar separation between two simultaneous independent final streams without procedural restriction to one stream only — structurally distinct from PRM-NTZ (closely-spaced PARALLEL no-transgression-zone, totally different geometry), STCA (global short-term conflict probe not approach-runway-pair specific), MTCD (medium-term 8-20min probe), CLAM/RAM (cleared-level / route adherence ground safety-nets), LAHSO (hold-short release authority — CRDA may be PREREQUISITE at LAHSO-active pairs but is structurally separate as LAHSO is a CLEARANCE category), APCH-CAT (ILS Cat I/II/III low-vis approach cert framework), TBS (Heathrow eTBS same-FAC time-based separation for one final stream), FIM-ASPA (flight-deck pairwise spacing on cockpit display, opposite side of the ATC/cockpit interface) — CRDA is uniquely the CONVERGING-RUNWAY GHOST-TARGET PROJECTION & TIE-POINT CONFORMANCE evaluator · 8-pair canonical CRDA tie-point catalogue documented in FAA facility orders KPHL 09R/17 (A11 PHL TRACON, 80° converge, KMOON tie-point, 4.8 NM stagger, LAHSO-active) / KBOS 22L/27 (A90 BOS, 50° converge, WAYNE tie, 4.0 NM stagger, LAHSO + VMC-only) / KORD 09R/04R (C90 ORD legacy pre-OMP plan-B geometry, 50° converge, 5.5 NM stagger) / KATL 26R/27R (A80 ATL same-direction independent close-parallel, 2.5 NM stagger only) / KSFO 28L/01L (NCT SFO 89° converge, SFO tie, 4.5 NM stagger, LAHSO + VMC-only legacy GA-deck Tipps procedures) / KLGA 22/31 (N90 NY 90° converge, LGA tie, 3.5 NM stagger, LAHSO + VMC-only) / KMCO 18R/17L (F11 MCO same-direction indep close-parallel, 2.0 NM stagger) / KDFW 35C/31R (D10 DFW plan-X non-standard 65° converge, 5.2 NM stagger, LAHSO) · 8-driver CRDA conformance scorer TIE-MISS (ghost projection lands inside required-min-sep ring at tie point = immediate breach score-min 90) / CMP-WAKE (RECAT-1.5 wake category requires upstream extra separation 4/5/6/7/8 NM RECAT-EU class A/B/C/D/E/F follower matrix that ghost spacing does not afford) / GS-DIVERG (ghost GS-projection diverges >25 kt from upstream actual GS, CRDA assumes equal Vapp so large divergence = unreliable ghost requiring controller manual fallback per §5-9-8) / STAGGER (aircraft inside staging volume but outside stagger window ±2.5 NM tunnel tolerance) / ANGLE (pair converge angle >100° voids CRDA authority per JO 7110.65 §5-9-7 unless facility waiver) / LAHSO (downstream is LAHSO release per AC 91-73B §5, upstream ghost must clear hold-short line ≥1000 ft before downstream cleared to land) / AMBI (two upstream targets project to within 1.0 NM of each other on downstream FAC creating ambiguous ghost-to-live mapping) / IMC-DEP (pair restricted to VMC per facility SOP and current visibility <5 SM or ceiling <2500 ft voiding CRDA authority) · composite max·0.65 + mean·0.35 × ADV-MUL with TIE-MISS hard escalator score-min 90 + time-to-tie ≤60s severity multiplier · 6 tiers BREACH ≥80 ghost crosses min-sep ring at tie → vector upstream off OR downstream go-around per §5-9-8 / CRITICAL ≥62 ghost predicted to violate within next 60s → vector authority pre-empt / STAGGER ≥42 stagger window breach → adjust downstream speed re-sequence / WATCH ≥22 nominal CRDA window monitor next sample / CLEAN <22 ghost projection clean full sep maintained / IDLE flight not inside any CRDA staging volume (>15 NM from tie or ground or FL>120) · MapLibre tier-coloured halo+pin on real upstream aircraft + ghost circle on downstream FAC + dashed link real→ghost + tie-point markers + FAC dashed extension lines + tier+label symbols · AIRCRAFT/PAIRS/MATRIX/METHOD tabs with full RECAT-1.5 6-class wake-sep matrix display · 3 sliders SCOPE 5-25NM / LOOK 20-180s / ADV× 0.4-2.0 · 6 layer toggles HALO/PIN/LBL/GHOST/LINK/FAC · synthetic per-airport IMC degradation (visibility + ceiling) from morning-fog hour-of-day cycle drives IMC-DEP detection · (FAA Order JO 7110.65BB §5-9 Converging Runway Operations / §5-5 Radar Sep 3.0 NM / §5-5-4 wake-turbulence RECAT / §5-9-7 CRDA Eligible Geometries / §5-9-8 CRDA Anomaly Resolution / §3-9 Runway Selection / FAA Order JO 7210.3DD §5-8 CRDA Facility Documentation Reqs / FAA Order JO 7110.118A LAHSO / FAA AC 90-129A §3 RECAT-1.5 / FAA AC 91-73B §5 LAHSO / FAA TI-9520-1 STARS Adaptation Spec §3.4 CRDA Subsystem / FAA NextGen NAS-IR-99002107 STARS CRDA Module IRD / MITRE MP14-002 CRDA Operational Concept / FAA Wm J. Hughes TC TM-2009/19 CRDA Performance Evaluation / ICAO Doc 4444 PANS-ATM §6.7 / Doc 9643 SOIR / Doc 9870 Runway Incursion §3 / Annex 14 Vol I §3.1.13 Converging Runways / NTSB AAR-91-08 LAX1493 USAir 1493 vs SkyWest 5569 KLAX 24L 1991-02-01 converging-runway taxi-into-landing fatal precedent for CRDA-equivalent tower coord / NTSB DCA15IA014 KLGA 13/22 2014-12-17 / ATSB AO-2015-084 YSSY 16R/25 converging-arrival conflict precedent)', showCrda, ()=>{ const nv=!showCrda; setShowCrda(nv); lsSet('ft-crda', nv) }],
                ['PINCAP · Pilot-Incapacitation / Two-Person-Cockpit-Rule / Locked-Door & Sole-Pilot Continuity Monitor · per-airframe live evaluator of the four flight-deck-occupancy subsystems that together determine whether a single-occupant cockpit is possible during flight and whether that single occupant remains fit-to-fly: (a) two-person-cockpit-rule policy compliance — the operator-level requirement that whenever one pilot leaves the flight deck a second qualified crew member (typically the senior cabin attendant) enters and remains on the flight deck, originating in EASA SIB 2015-04 / FAA SAFO 15006 response to Germanwings 9525 (D-AIPX Barcelonnette 2015-03-24, 150 fatal, BEA Final 2016-03-13) and revised by EASA SIB 2016-09 + AAIB AAR-2017-05 to operator-discretion based on operator safety case (b) cockpit-access-denial dwell time — the locked armoured-door rejection-loop state under RTCA DO-329 / EUROCAE ED-198 + 14 CFR §25.795(a) intrusion-resistant flight-deck door spec (post-9/11 hardening per US PL-107-71 ATSA §104 + ICAO Annex 6 Pt I §13.2.2 + EASA AMC 25.795) with door-access dwell-time ≥459 s = Germanwings precedent triggering Cat-A escalation pathway (c) sole-pilot interval risk — single-pilot flight-deck dwell-time per-second probability of medically-significant incapacitation per Mitchell-Evans BMJ 2004 epidemiology (~1 event per 4M flight-h Class-1 ATPL × age curve 0.4-3.2/M-h × Caldwell ASEM 80-1 2009 fatigue elevation ×1.7 in WOCL) (d) subtle-incapacitation cue scoring — flightpath-deviation rate (|Δh|>100ft/min off-FMS), no-input-detected timer (autopilot in command no SP-side input ≥120s per FAA SAFO 17007 dual-pilot vigilance protocol), autopilot mode-reversion without acknowledgement (Boeing FCOM PR 9.20 / Airbus FCTM PRO-NOR-SOP-15), silence-on-frequency dwell (no readback within 30s per ICAO Annex 10 Vol II §5.2.1.4.1.1 / Doc 9432) (e) MAYDAY-PAN-INCAP downlink readiness — CPDLC FANS-1A+ link active per DO-258A + SELCAL HF-uplink per ARINC 596 + ACARS UTC-keyed crew-status downlink path per Boeing FCOM CHAP 17 / Airbus FCTM PRO-COM-50 enabling surviving pilot or cabin-crew relay to transmit MAYDAY-PAN within ICAO Annex 10 Vol II §5.3.1 60s emergency-broadcast window · structurally distinct from CIRCAD-FATIGUE (chronobiology / SAFTE / WOCL only — does not score occupancy or door state), CREW-DUTY (FAR-117 FDP/duty-time book-keeping not real-time vigilance), TUC-HYPOXIA (physiological time-of-useful-consciousness post-decompression only), MEDLINK-DIVERSION (passenger medical not crew), VESTI-SPATIAL (somatogravic disorientation regime), SCRM-STERILE (sterile-cockpit non-essential conversation below 10k ft), COCKPIT-HUD (symbology not occupant), EFB-MONITOR (chart-data currency) — PINCAP uniquely sits at FLIGHT-DECK OCCUPANCY + LOCKED-DOOR + INCAPACITATION-CUE + MAYDAY-DOWNLINK intersection, the Germanwings 9525 / Helios 522 / LAM-470 problem class · 12-class crew-policy catalogue NB-DOM-US 2P REQ DO-329 / NB-INT-US 2P REQ DO-329 / NB-EU 2P OP-DISC ED-198 / WB-MED 2P OP-DISC ED-198 / WB-LH-3P 3P REQ DO-329 / WB-ULR-4P 4P REQ DO-329 / RGN-J 2P OP-DISC ED-198 / TURBO 2P OP-DISC ED-198 / BIZ 2P WAIVED BASIC / CARGO-LH-3P 3P N/A DO-329 / HEMS 1-2P NONE / LIGHT 1P N/A · per-flight synthetic state model deterministic FNV-1a hash on icao24 + day-seed matching post-2015 SOP + Mitchell-Evans incapacitation distribution (88% BOTH / 8% SINGLE-LAV / 3.5% SINGLE-REST / 0.5% EMPTY-LOCKED · 94% SECURED / 3% GRANTED / 2.5% DENIED-RING / 0.5% OVERRIDE) with synthetic accessDwellS/soleDwellS/noInputDwellS/deviationRate/trackDevDegMin/ackMissed/freqSilenceS sampler · 8-phase classifier TKO-INI 1.30 / CLIMB 1.10 / CRUISE 1.00 (lav-trip peak) / STEP-CLIMB 0.95 / DESCENT 1.05 / APPR-INT 1.20 / APPR-FNL 1.40 (high-workload LOC-I cue cluster peak) / GROUND 0.40 · 8 drivers OCC (occupancy-state penalty BOTH=0 / SINGLE-LAV=22 / SINGLE-REST=14 / EMPTY-LOCKED=100) / DWELL (sole-pilot dwell-seconds ramp 0→100 over 0..600s Mitchell-Evans baseline) / DOOR (access-dwell ramp 0→100 over 0..480s Germanwings 459s precedent) / CUES (subtle-incap cues composite deviation+track-Δ+no-input+ack-missed) / FREQ (frequency-silence 0..120s past-due readback) / POLICY (two-person-rule gap × current single-occupancy REQ=60 OP-DISC=30 WAIVED=0) / DOWN (MAYDAY-downlink readiness NONE=100 ACARS-ONLY=55 SELCAL-ACARS=25 CPDLC-SELCAL-ACARS=0) / FAT (CIRCAD bridge effectiveness deficit) · composite max·0.62 + mean·0.38 × phase-w × ADV-MUL · hard escalators (a) doorState=DENIED-RING + accessDwell ≥459s + REQ-class → 96 GERMANWINGS-MODE Cat-A EASA SIB 2015-04 escalation (b) occ=EMPTY-LOCKED any phase → 95 LAM 470 2013 precedent autopilot-only (c) DENIED-RING + accessDwell ≥120s → 84 RING-DENIED cabin-crew repeat-call SOP ED-198 §4 (d) SINGLE-LAV + DWELL ≥soloFloor + CRUISE → 72 LAV-OVERRUN per Boeing FCOM PR 9.10 (e) CUES ≥60 in APPR-INT/FNL → 78 subtle-incap brief takeover per FAA SAFO 17007 + Helios 522 precedent (f) FREQ silence ≥45s in APPR-FNL → 60 readback overdue per Annex 10 §5.2.1.4.1.1 · 6 tiers GW-MODE ≥85 rose Cat-A flightpath-protection / LOCKED-OUT ≥65 rose-pink door rejection-ring active repeat-call cabin alert / SUB-INCAP ≥45 amber subtle cues brief takeover / SOLO-WATCH ≥22 sky single-pilot dwell exceeds prudent envelope / NOMINAL <22 emerald two-pilot vigilant deck / OFF on-ground or single-pilot-class slate · MapLibre overlay 5 layers pincap-halo tier-coloured ring 7-19px score-sized / pincap-pin GW-MODE/LOCKED-OUT rose pin / pincap-door dashed-rose 30px orbit ring on DENIED-RING+accessDwell>120s showing locked-out cabin-crew presence / pincap-vec dashed forward subtle-incap deviation vector along track (length proportional to CUES × phase-w × 0.05 km) / pincap-lbl callsign · class · OCC · soleDwell · TIER · Side panel sticky header 6-tier counter strip click-to-filter ALL + 5-cell summary μ-SCORE / GW-cnt / LOCK-cnt / SOLO-cnt / WORST-cs + 4 sliders ADV-MUL 50-200% / SOLO-FLOOR 60-600s / CUE-MUL 50-200% / DOOR-MUL 50-200% + 8-phase chip filter + 12-class chip filter + 5 toggles HALO/PIN/DOOR/VEC/LBL + cs/type/operator/class search + 4-tab switcher AIRCRAFT per-airframe row stack tier-worst-first cs+type+class-pill+phase-pill+tier-pill+OCC-pill+DOOR-pill + DWELL/CUES/FREQ row + 8-driver chips + tier-coloured advice citing reg + SYSTEMS per-class equipment / 2P-rule / door-spec / downlink-class with fleet μ-SCORE+GW-cnt+LOCK-cnt + DRIVERS 8-driver fleet-mean bars + phase-weight card + Mitchell-Evans epidemiology rate table + PRECEDENT 6 canonical case studies Germanwings 9525 / Helios 522 / LAM 470 / EgyptAir 990 / Silk Air 185 / Continental 1404 with per-driver attribution rosette · PINCAP entry registered in Layers Safety & Traffic category after CRDA, ft-pincap persisted preference · References: EASA SIB 2015-04 / 2016-09 / FAA SAFO 15006 / 17007 / ICAO Annex 6 Pt I §13.2.2 / Annex 10 Vol II §5.2.1.4.1.1 §5.3.1 / Doc 9432 / Doc 9985 §3.4 / Doc 9966 App.B / 14 CFR §25.795(a) §121.587 §121.583 §117.17 §117.19 §135.99 §91.105 / US PL-107-71 ATSA §104 / EASA AMC 25.795(a) / CS-25.795 / AMC1 ORO.SEC.100 / RTCA DO-329 / EUROCAE ED-198 / RTCA DO-258A / ARINC 741 / ARINC 596 / UK CAA CAP 715 §3 / TC AC 705-001 §6 / CASA CAO 20.18 §10 / IATA SeMS ed.5 §4 / IATA COS-BP §6 / Boeing FCOM Vol 1 §17 / FCTM Ch 3 §3.20 / FCOM PR 9.10 9.20 / FCOM CHAP 17 / Airbus FCOM DSC-25 / FCTM PRO-NOR-SOP-15 / PRO-COM-50 / PRO-SUP-91 / FCOM PRO-ABN-MISC / Embraer AOM §03 / Bombardier FCOM §16 / BEA Germanwings 9525 D-AIPX 2016-03-13 / AAIB Cyprus Helios 522 5B-DBY AAR-11/2006 / CIAIM Mozambique LAM 470 C9-EMC 2016 / NTSB EgyptAir 990 SU-GAP AAR-02/01 / NTSC + NTSB Silk Air 185 9V-TRF / NTSB DCA09MA021 Continental 1404 N18611 / Mitchell-Evans BMJ 2004;329:1235 Cardiovascular Incapacitation of Airline Pilots / Caldwell ASEM 80-1 2009 / Bennett Pilot Incapacitation Routledge 2018 Ch.4 / Veillette FlightSafety AeroSafetyWorld 2015-06 / Lufthansa OM-A 2018-05 / Eurocontrol EVAIR 2019 Subtle Incapacitation chapter', showPincap, ()=>{ const nv=!showPincap; setShowPincap(nv); lsSet('ft-pincap', nv) }],
                ['EICAS · ECAM / EICAS Multi-Failure Master-Caution / Warning Alert-Cascade & Crew-Workload Stack Monitor · per-airframe live evaluator of the cockpit ALERT-DISPLAY subsystem during multi-failure / cascade events scoring simultaneous Master-Warning / Master-Caution / Advisory message count, cascade DIRECTION (CONVERGE / STATIC / DIVERGE / RUNAWAY) measured as net new-alert rate per minute, alert-priority sort INVERSION (fraction Level-3 red Master-Warning masked by Level-2 amber spam per SAE ARP-5588), unacknowledged-top-priority dwell-time, cumulative ECAM/QRH action-items per minute vs crew cadence baseline (FAA AC 25.1322-1 §6.2.4 task-allocation limits), electronic-checklist nesting depth, and latent + active fault overlap (Helios 522 canonical mode) against the per-family alert-architecture envelope · 13 airframe alert-architecture families catalogued each with own saturation threshold (satN), crew response cadence items/min, ECL nest tolerance, latent susceptibility, and priority-inversion baseline: B-EICAS-NG B777/B787 EICAS + Electronic Checklist coupling (Boeing FCOM Vol 2 §02 / 787 OM §0.05) satN=10 cadence=7.5/min ECL≤4 latS=10% · B-EICAS-CL B757/B767/B747-400 classic CRT EICAS satN=8 cad=6.0 · B-EICAS-MAX B737 MAX upgraded EIS satN=9 cad=6.8 · B-EICAS-737CL B737NG individual annunciator-light EIS satN=6 cad=5.0 latS=32% · A-ECAM-NG A350/A380 next-gen smart-page parallel-procedure ECAM (Airbus FCOM PRO-NOR-SOP-21) satN=12 cad=8.0 latS=8% · A-ECAM-FBW A320/A330/A340 FBW ECAM with FWC LEVEL-3 red MW / LEVEL-2 amber MC / LEVEL-1 advisory architecture (FCOM DSC-31 / PRO-ABN-MISC) satN=10 cad=7.0 · A-ECAM-CL A300/A310 classic CRT FCU/FWC era satN=7 cad=5.5 · A-ECAM-ATR ATR-42/72-600 Thales Avionica adapted ECAM (FCOM §2.07) · E-EICAS-E2 Embraer E2 next-gen enhanced fault-handling · E-EICAS-IV Embraer E1 Honeywell Primus Epic EICAS-IV (POM §0.04) satN=8 cad=6.0 · C-EICAS-CRJ Bombardier CRJ-700/900 Collins Pro-Line 4/21 satN=7 cad=5.5 · C-EICAS-Q DHC-8-Q400 simplified EICAS · O-OTHER mixed/GA baseline · 7 driver scores 0..100 aggregated per flight: MSG-COUNT (simultaneous W+C+A vs per-family satN saturation, ≥4 red MW floors 80, ≥2 reds 55) / CASCADE (net new-alert rate per minute scaled to direction: CONVERGE×0.25 STATIC×0.60 RUNAWAY-min 85) / PRIO-INV (inversion fraction mapped 0..1→0..100 per ARP-5588) / ACK-LAG (top-priority unacknowledged seconds, 0-5s shallow then steep beyond 30s per Sarter-Woods IJAP 1995 cognitive engagement) / WKLD-RATE (cumulative action items/min vs cadence baseline ratio; <40%=light 40-90%=moderate 90-120%=heavy >120%=saturation per AC 25.1322-1 §6.2.4) / ECL-DEPTH (nest depth vs family eclMax tolerance then escalator) / LATENT (boolean+family susceptibility coefficient 60+susceptibility×130) · composite max(0.65·worst-driver, 0.35·weighted-mean) + 0.35·weighted-mean then × phase multiplier (CASCADE-PH 1.25 / TKO|LDG 1.15 / APP 1.10 / CLB 0.90 / DST 0.92 / CRZ 0.85 / GND 0.55) · hard escalators ≥5 red MW → composite floor 88 / RUNAWAY direction → floor 80 / phase auto-elevates to CASCADE-PH when scenario is UER-LIKE-QF32 / HYD-LIKE-UA232 / UAS-LIKE-AF447 / EIS-LIKE-KEGWORTH · 6 tiers BAN ≥85 rose SATURATION crew at workload-saturation comprehension breakdown imminent declare emergency + transfer-of-control + ATC hold + delegate triage / CRIT ≥65 rose-pink CASCADE alert generation > response cadence sterile-cockpit prioritise top-3 Level-3 alerts only / MARGIN ≥45 amber HEAVY-LOAD multiple parallel ECAM actions PF/PM colour-band split / WATCH ≥25 sky SINGLE-FAULT single ECAM/QRH actioned per established flow / CLEAR ≥10 emerald MINOR advisory only / OFF <10 slate ROUTINE no active alerts · accident precedent library seeded into 11 scenarios sampled deterministically per ICAO hash: ROUTINE 70% / SINGLE-CAUT 13% / SINGLE-WARN 6% / MULTI-PARALLEL 5% / UER-LIKE-QF32 2.2% (A380 VH-OQA 2010-11-04 ATSB AO-2010-089 Trent-900 #2 IPC stage-2 disk uncontained burst 54 ECAM messages 22 systems compromised 80 min of continuous ECAM action items Capt deCrespigny 5 on flight deck triaged via ECAM colour-line prioritisation + delegated tasks canonical multi-failure cascade case) / HYD-LIKE-UA232 1.3% (DC-10 N1819U 1989-07-19 NTSB AAR-90-06 CF6-6 #2 fan-disk uncontained failure severed all three hydraulic systems massive cascade Sioux City emergency landing 184/296 survived) / UAS-LIKE-AF447 1.0% (A330 F-GZCP 2009-06-01 BEA F-CP090601 pitot icing UAS cascade FBW Alt-2 reversion inconsistent stall warns crew never consolidated mental model) / MODE-LIKE-OZ214 0.7% (777 HL7742 2013-07-06 NTSB AAR-14-01 A/T-mode masking low-speed FLCH-SPD-HOLD reversion canonical PRIO-INV reference) / LATENT-LIKE-HELIOS 0.3% (737-300 5B-DBY 2005-08-14 AAIASB 11/2006 pre-departure pressurization mode-switch latent + cabin-alt warn horn ≡ TO-config horn signature crew hypoxia canonical LATENT reference) / TOWS-LIKE-5022 (MD-82 EC-HFP 2008-08-20 CIAIAC A-032/2008 TOWS silent C/B failure pre-rotation canonical inhibit-failure mode) / EIS-LIKE-KEGWORTH 0.5% (BMA 092 737-400 G-OBME 1989-01-08 AAIB 4/90 fan-blade-out crew mis-read new EIS engine display shut down OTHER good engine canonical PRIO-INV mis-localisation) · MapLibre overlay tier-coloured outer halo radius=6+score×0.22 + BAN/CRIT outer cascade ring + BAN/CRIT/MARGIN pin + BAN/CRIT cs+family+msgcount labels coloured by family swatch · AIRCRAFT/FAMILIES/PRECEDENT/METHOD tab switcher · 6-tier counter strip click-to-filter ALL + 4-cell summary μ-Score / Worst-cs / Cascade-count / Latent-count + 3 sliders ADV-MUL 50-200% / MIN-FL 0-200 / MAX-FL 50-450 + 13-family chip filter B-NG/B-CL/B-MAX/B-737CL/A-NG/A-FBW/A-CL/A-ATR/E-E2/E-IV/C-CRJ/C-Q/OTHER + 8-phase chip filter GND/TKO/CLB/CRZ/DST/APP/LDG/CASCADE-PH + HALO/PIN/LBL/RING toggles + search by cs/type/family/scenario · AIRCRAFT tier-worst-first row stack with cs+type+family-pill+phase-pill+tier-pill + FL/kt/VS line + W/C/A msg-counts coloured red/amber/sky + cascade-direction symbol+rate + ECL/ACK indicators + LATENT badge + tier-coloured score bar + 7-driver chip grid MSG/CASC/PRIO/ACK/WKLD/ECL/LAT + tier-rationale advice citing specific FCOM ref + scenario precedent quote · FAMILIES per-family row with FAM-NG/CL spec satN/cadence/ECL-max/latS readout + active-aircraft count + ban+crit count + cascading count + tier-coloured mean-score bar · PRECEDENT per-scenario row with current-tick sample count + full accident citation · METHOD full prose on driver weights/composite/tier mapping/precedent library/distinctions from FMA/FBW-REV/MCAS/MEL/EHM/PINCAP/TUC/PIO + full reference list · structurally distinct from FMA (automation-mode confusion single subsystem Asiana 214 / Turkish 1951), FBW-REVERSION (FBW law degradation Normal/Alt-1/Alt-2/Direct/Mech single longitudinal-feel), MCAS / STAB-TRIM (B737-MAX pitch-augmentation runaway single failure), MEL / CDL (dispatch-time deferral pre-flight), EHM / OIL / VIB / EGT (single-engine parameter trending), PINCAP (human crew incapacitation two-person-cockpit rule), TUC (hypoxia / O₂ depletion), PIO (closed-loop pilot-airframe coupling high-gain compensatory tracking) — EICAS-CASCADE is uniquely the MULTI-FAILURE ALERT-DISPLAY SATURATION evaluator · (14 CFR §25.1322 Flight crew alerting / EASA CS-25.1322 / FAA AC 25.1322-1 2010-12-13 / AC 25-11B / EASA AMC 25.1322 ED 2013/010/R / SAE ARP-4754A / ARP-4761/A §A.3 / ARP-5588 / ARP-4102/4 / AIR-6055 / ICAO Annex 6 Pt I §6.10 / Doc 9760 Vol II Pt VI / Doc 9859 SMM ed.4 §2 / Doc 9683 HF Training / FAA-H-8083-3C Ch.17 / Boeing D6-83563 EICAS 777 / FCOM Vol 2 §02 / 787 OM §0.05 / Airbus FCOM DSC-31 ECAM Architecture / PRO-ABN-MISC / PRO-NOR-SOP-21 / Embraer EICAS-IV POM §0.04 Honeywell / CRJ FCOM Vol 2 §02 / ATR FCOM §2.07 / RTCA DO-178C / DO-254 / DO-275 / Sarter-Woods IJAP 1995 Strong Silent Type / HF 1997 mode confusion / Wickens HF 2008 multitask workload / FSF ALAR BN 7.1 / IATA STEADES 2024 §11 / EUROCONTROL EVAIR Bull 28 §4 / NTSB AAR-90-06 UA232 / AAR-14-01 OZ214 / AIR-08-01 Spanair 5022 / ATSB AO-2010-089 QF32 / BEA F-CP090601 AF447 / AAIB 4/90 Kegworth / AAIASB 11/2006 Helios 522 / CIAIAC A-032/2008 Spanair · EICAS entry registered in Layers Safety & Traffic category after PINCAP, ft-eicas persisted preference)', showEicas, ()=>{ const nv=!showEicas; setShowEicas(nv); lsSet('ft-eicas', nv) }],
                ['UPRT · Upset Prevention & Recovery Training Envelope & Aircraft-Upset / Unusual-Attitude State Classifier Monitor · per-airframe live evaluator of the IATA/ICAO/FAA Aeroplane Upset Prevention & Recovery Training Aid (AUPRTA) Rev 3 5-axis attitude/energy envelope as defined post-AF447 / post-Colgan-3407 to address Loss-of-Control In-flight (LOC-I) as the #1 CAST commercial accident category by fatality count over the past two decades · the AUPRTA / ICAO Doc 10011 / FAA AC 120-111 / EASA AMC1 FCL.745.A / IATA APS UPRT 2nd-ed definition of "airplane upset" is the per-frame envelope: pitch > 25° NU / pitch > 10° ND / bank > 45° / IAS inappropriate for current configuration (>1.10·VMO or <1.13·Vs1g) / α approaching α_stall PLUS any inappropriate flight-path response, dynamic divergence, or automation-induced excursion · 9-class airframe taxonomy with per-class certification envelope (vs1g/vmo/mmo/ngLim/α_stall/pitchEnv/bankEnv): WB-HVY B777/B787/A350/A380 ngLim 2.5g M_MO 0.92 · WB-T2 B767/A330/A340 ngLim 2.5g M_MO 0.88 · NB B737/A320/A321/A220 ngLim 2.5g M_MO 0.84 · RGN-J E190/CRJ900 ngLim 2.5g · RGN-T ATR72/Q400 V_MO 245kt · BIZ G650/GLEX/Falcon ngLim 2.8g M_MO 0.90 · MIL C-17/KC-135 pitchEnv ±30°/15° · LIGHT PC-12/TBM/Cirrus ngLim 3.8g · OTHER · 7 driver scores 0..100 aggregated per flight: ATT-PITCH (signed pitch vs 25° NU / 10° ND AUPRTA thresholds, escalator above) / ATT-BANK (|bank| vs 45° AUPRTA threshold, >60° hard escalator) / ENERGY (KE+PE balance speed-appropriate-for-altitude proxy, low-energy <0.85·Vs1g·1.10 and high-energy >0.95·VMO both score) / MARGIN-V (Vs1g·1.13 lower / 0.97·VMO upper explicit envelope proximity + 0.97·MMO compressibility) / G-LOAD (Δn deviation from 1g + corner-loading vs §25.337 ngLim) / AUTOM (automation-induced excursion risk · mode-confusion driver per Sarter-Woods IJAP 1995 / FMA) / ENV (turbulence-EDR + wake + wind-shear + icing + mountain-wave aggravator multiplier) · 8 upset categories AERO-STALL / HIGH-PITCH-NU / HIGH-PITCH-ND / HIGH-BANK / UNDER-SPEED / OVER-SPEED / DUAL-ENV / NEUTRAL · AUPRTA Rev 3 5-class generative mechanism taxonomy AERODYNAMIC (Class-A · stall / α-departure / control-surface authority loss) / SYSTEM-INDUCED (Class-B · RTLU / PCU hardover / ADIRU / IRS dual fault) / ENVIRONMENT (Class-C · turbulence / wake / wind-shear / mountain-wave / icing) / PILOT-INDUCED (Class-D · somatogravic / startle / Birgenair pitch-up) / AUTOMATION-INDUCED (Class-E · automation surprise / mode confusion / Alt-Law reversion) · 9-phase classifier TKO / INIT-CLB / CLB / CRZ / DST / APP-INT / APP-FNL / FLARE / GA / OFF with phase-mul TKO 1.10 / INIT-CLB 1.25 / CLB 1.05 / CRZ 1.00 / DST 1.00 / APP-INT 1.10 / APP-FNL 1.35 / FLARE 1.50 / GA 1.40 (INIT-CLB & FLARE are highest LOC-I exposure phases per CAST 2019-2023 analysis) · composite max(0.65·worst-driver, 0.35·weighted-mean) + 0.35·weighted-mean × phase-multiplier · hard escalators |pitch|>30° NU or >15° ND → composite-floor 88 / |bank|>60° → 85 / α≥α_stall AERO-STALL warning → 80 / DUAL-ENV ≥2-concurrent-corners → 78 · 6 tiers CRITICAL ≥85 rose ACTIVE UPSET PENETRATION apply recovery push/roll/thrust/pitch FCTM 8.5 / RECOVERY ≥65 rose-pink AUPRTA recovery procedure required / UPSET ≥45 amber envelope corner approached verify FMA monitor pitch/bank re-stabilise on attitude indicator / MARGIN ≥25 sky elevated risk monitor energy state / CLEAR ≥10 emerald within AUPRTA envelope / NEUTRAL <10 slate benign cruise · LOC-I accident precedent library seeded into 14 scenarios sampled deterministically per ICAO hash: NORMAL ~85-88% in cruise / MILD-TURB 6-8% / NEAR-STALL 3-5% on approach / BANK-WAKE 1-3% / PITCH-NU-GA 0.5% / AF447-LIKE 0.5% (A330 F-GZCP 2009-06-01 BEA F-CP090601 228 fatal canonical AUTOM+AERO-STALL UAS cascade → ALT-LAW → aft sidestick held → 30° NU pitch + 16° bank → full stall to ocean surface FL350 → 0) / COLGAN-LIKE 0.5% (Q400 N200WQ 2009-02-12 NTSB AAR-10-01 50 fatal canonical AERO-STALL stick-shaker @ flap/gear → captain pulled aft against shaker → stick-pusher inhibited → 31° NU + 105° L bank → impact) / AIRASIA-LIKE 0.4% (A320 PK-AXC 2014-12-28 KNKT 32-12-12 162 fatal RTLU rudder-limiter fault → 5× MASTER CAUTION → CB reset killed FAC1+2 → ALT-LAW → 54° L bank → 38° NU stall → loss to Java Sea) / ABX-LIKE 0.3% (B767-300 N1217A 2019-02-23 NTSB DCA19MA086 3 fatal canonical PILOT-INDUCED somatogravic ND upset GA-mode inadvertent activation on flap extension → pilot pushed nose down attributing pitch-up to autoflight → 49° ND → impact) / USAIR427-LIKE 0.2% (B737 N513AU 1994-09-08 NTSB AAR-99-01 132 fatal rudder PCU jam-reversal full hardover → uncommanded roll → split-throttle → ND penetration → impact) / CHINAAIR006-LIKE 0.3% (B747SP N4522V 1985-02-19 NTSB AAR-86-03 0 fatal engine #4 flame-out + manual A/P retention → uncoordinated roll → 270° rolling dive FL410 → FL110 recovery g-load 4.8g max +5g cert canonical recovery survival case) / ADAM574-LIKE 0.2% (B737-400 PK-KKW 2007-01-01 KNKT 07-26-VWB 102 fatal IRS dual fault → captain head-down troubleshoot → roll departure → 100° R bank → impact ocean) / BIRGENAIR-LIKE 0.2% (B757 TC-GEN 1996-02-06 DGAC Dom Rep 189 fatal pitot static blockage UAS → A/T speed conflict → pitch-up stall departure to ocean) / TATARSTAN-LIKE 0.2% (B735 VQ-BBN 2013-11-17 IAC final 50 fatal inadvertent go-around → captain trim error + pitch-up runaway → 75° ND penetration → impact) · MapLibre overlay tier-coloured outer halo radius=6+score·0.22 + CRITICAL/RECOVERY outer cascade ring + CRITICAL/RECOVERY/UPSET pin + CRITICAL/RECOVERY cs+category+pitch°+bank° labels coloured by category swatch · AIRCRAFT/CATEGORIES/MECHANISMS/PRECEDENT/METHOD tab switcher · 6-tier counter strip click-to-filter ALL + 4-cell summary μ-Score / Worst-cs / Crit+Rec count / Dual-Env count + 4 sliders ADV-MUL 50-200% / ENV-MUL 50-250% / MIN-FL 0-200 / MAX-FL 50-450 + 8-category chip filter STALL/HP-NU/HP-ND/HBK/USPD/OSPD/DUAL/NTR + 9-phase chip filter (excl OFF) + 9-class chip filter WB-HVY/WB-T2/NB/RGN-J/RGN-T/BIZ/MIL/LIGHT/OTHER + HALO/PIN/LBL/RING toggles + search by cs/type/category/mechanism/scenario · AIRCRAFT tier-worst-first row stack with SVG attitude-indicator badge (live pitch/bank) + cs+type+class-pill+category-pill+phase-pill+tier-pill + FL/IAS/Mach/VS line + signed-pitch / bank / α / g-load 4-cell coloured by envelope-corner proximity + WAKE/RCV-AGL flag chips + tier-coloured score bar + 7-driver chip grid PITCH/BANK/ENERGY/MARGIN/GZ/AUTOM/ENV + tier-coloured rationale advice citing FCTM 8.5 recovery procedure + scenario precedent quote click-to-fly · CATEGORIES per-category row with cat-coloured swatch + AUPRTA description + crit+rec count + tier-coloured mean-score bar · MECHANISMS AUPRTA Rev 3 5-class taxonomy row with class-description + flight-count tally · PRECEDENT per-scenario row with current-tick sample count + full accident citation (operator/registration/date/fatality/investigation-body) · METHOD full prose on driver weights/composite/tier mapping/precedent library/distinctions from STALL/DEEPSTL/DUTCH-ROLL/MACH-TUCK/PIO/VMC/TEM-ENERGY/FMA/COFFIN/UAS + full reference list · structurally distinct from STALL (single-axis 1g α-floor warning — STALL is one driver inside UPRT not the envelope), DEEPSTL (post-stall T-tail Cm(α) reversal locked-in — specific catastrophic exit-mode), DUTCH-ROLL (lateral-directional eigenmode damping not attitude excursion), MACH-TUCK (transonic Cm(M) reversal at M_crit — driver of HIGH-PITCH-ND class not envelope), PIO (closed-loop pilot-airframe Neal-Smith Bandwidth/Phase-Delay coupling — UPRT is OPEN-loop attitude envelope departure), VMC/VMCA/VMCL (asymmetric-thrust controllability floor lateral-directional driver), TEM-ENERGY (KE+PE balance state monitor — contributing driver does not score attitude), FMA (automation mode-awareness Sarter-Woods — driver of AUTOM-INDUCED class), COFFIN (Vmo-Mmo/Vs1g convergence cruise envelope only no attitude), UAS/pitot-icing (sensor failure cascade — specific sub-driver) — UPRT is uniquely the AGGREGATED airplane-upset envelope classifier per AUPRTA Rev 3 5-class mechanism taxonomy: it is the INTEGRATION LAYER above the seven single-axis monitors · (ICAO Doc 10011 Manual on Aeroplane Upset Prevention & Recovery Training 2014 / FAA AC 120-111 UPRT 2015-04-14 / FAA AC 120-109A Stall Prevention & Recovery Training 2015 / FAA Order 8900.1 Vol 3 Ch 19 §11 / 14 CFR §121.423 / §121.424 / §61.155 / §61.157 / §25.143 (g)/(h) / §25.181 / §25.203 / §25.255 / EASA Part-FCL AMC1 FCL.745.A APS UPRT / CS-FCD AMC1 ORO.FC / ICAO Annex 1 Ch 2 / Doc 9868 PANS-TRG App 2/3/4 / Doc 9683 HF training / Doc 9994 EBT manual / IATA APS UPRT Guidance Material 2nd ed. 2019 / IATA LOC-I Accident Analysis Report 2019-2023 / CAST/ICAO CICTT LOC-I category / IATA/Boeing/Airbus AUPRTA Rev 3 2017 / Royal Aero Soc UPRT FAQ / ICATEE Industry Committee on UPRT / NTSB AAR-10-01 Colgan 3407 / AAR-86-03 China Air 006 / AAR-99-01 USAir 427 / DCA19MA086 ABX 3591 / BEA F-CP090601 AF447 / KNKT 32-12-12 AirAsia 8501 / KNKT 07-26-VWB Adam Air 574 / ATSB AO-2008-070 Qantas 72 ADIRU upset / NTSB AAR-09-04 UPS 1354 / Boeing FCTM 8.5 Upset Recovery / Airbus FCTM PRO-ABN-MISC Upset Recovery / Embraer AOM §03 / CRJ FCOM Vol 2 §03 / IATA STEADES 2024 §3 / FAA/NASA TM-2018-219997 LOC-I analysis / FAA-S-8081-22 PTS ATP type-rating UPRT tasks · UPRT entry registered in Layers Safety & Traffic category after EICAS, ft-uprt persisted preference)', showUprt, ()=>{ const nv=!showUprt; setShowUprt(nv); lsSet('ft-uprt', nv) }],
                ['TLS · Target Level of Safety · Reich Collision-Risk-Model · per-airframe probabilistic pair-wise CRM evaluator that estimates the expected MAC-rate-per-flight-hour for each airborne aircraft against the canonical Reich-Marks 1966 framework (Reich PG 1966 "A Theory of Safe Separation Standards for Air Traffic Control" J.Inst.Navig 19 #1/#2/#3 + Marks BL 1963 RAE Tech Note 91) — the bedrock probabilistic model that ICAO Annex 11 Para 3.1.4 / Doc 9689 Manual on Airspace Planning Methodology for Determination of Separation Minima §3.4 uses to JUSTIFY every horizontal and vertical separation standard in en-route, terminal, and oceanic airspace and to evaluate the ICAO TLS target 5×10⁻⁹ MAC-rate/h opposite-direction parallel route (or 1.5×10⁻⁸/h bidirectional same-track) · the Reich-Marks CRM expresses the expected mid-air-collision rate per flight-hour for an aircraft of semi-dimensions {λx, λy, λz} (60m × 60m × 17m canonical Reich values) as the sum of three pair-wise passing-rate × overlap-probability terms: N_x = Py(0)·Pz(0)·|Δv̄|/(2λx) longitudinal / N_y = Px(0)·Pz(0)·|Δv_y| lateral / N_z = Px(0)·Py(0)·|Δv_z| vertical · where Px/Py/Pz are conditional overlap probabilities at zero relative displacement (the Reich overlap kernels) implemented as Gaussian kernels evaluated at observed pair displacement convolved with combined navigation/altimetry σ (RNP × 0.5 lateral / ASE per Doc 9574 Table 3.A.1 vertical) — the aircraft pair is "in collision" when |Δx|<λx AND |Δy|<λy AND |Δz|<λz · 8-class airframe taxonomy with per-class navspec performance: WB-RNP4 B777/B787/A350/A380 GNSS+IRS+SBAS RNP-4 oceanic / WB-RNP10 B767/A330/A340 GNSS+IRS RNP-10 legacy / NB-RNP1 B737NG/B73M/A320/A321 GNSS+SBAS RNP-1 PBN TMA / RGN-RNP1 E190/CRJ900 GNSS+SBAS PBN / BIZ-RNP4 G650/GLEX/Falcon oceanic / TURBO-BRNAV ATR72/Q400 RNAV-5 / LIGHT-BASIC PC12/TBM/C172 GNSS-WAAS / OTHER · 7 driver scores [0..100] aggregated per flight: SX-LONG (Reich longitudinal passing-rate × Py(0)·Pz(0) overlap kernel · along-track relative-velocity term) / SY-LAT (Reich lateral passing-rate × Px(0)·Pz(0) overlap kernel · route-keeping deviation term) / SZ-VERT (Reich vertical passing-rate × Px(0)·Py(0) overlap kernel · altimetry-system-error term) / DENS (local 100NM cylinder traffic density proportional to encounter rate λ in CRM) / REGIME (airspace regime RVSM / RNP-1 / RNP-4 / RNP-10 / random-route oceanic) / NAV-PERF (per-airframe RNP/RNAV containment performance) / SURV (surveillance performance class radar-5NM / radar-3NM / ADS-B / Multilat / procedural) · 8 risk categories by dominant axis: VERT-RVSM (vertical overlap dominant · RVSM altimetry-system-error regime · Reich Pz(0) kernel · Doc 9574) / VERT-LARGE (vertical overlap dominant · non-RVSM 2000ft separation transitional) / LAT-RNP1 (lateral overlap dominant · RNP-1 PBN terminal-area 95%/99.999% containment) / LAT-RNP4 (lateral overlap dominant · RNP-4 oceanic 23NM/23NM lat/long Doc 9613) / LAT-RNP10 (lateral overlap dominant · RNP-10 oceanic 50NM/50NM legacy GNSS/INS) / LONG-TRAIL (along-track in-trail dominant same-direction speed-difference passing-rate) / OCEAN-LONG (oceanic NAT/PACOTS time-based 10min/15min same-track sep at fixed flight level) / CLEAR (no significant neighbor pair · isolated airspace) · 8-phase regime classifier: ENROUTE-RVSM FL290-FL410 1000ft sep / ENROUTE other 2000ft sep / TMA <FL100 3NM sep / OCEANIC-PBN RNP-4/10 with CPDLC/ADS-C / OCEANIC-RNDM random-route 50NM lat / CLB-TRANS FL280-300 RVSM gate / DST-TRANS FL280-300 RVSM gate / GND · phase-mul ENROUTE-RVSM 1.05 / ENROUTE 1.00 / TMA 1.20 / OCEANIC-PBN 0.85 / OCEANIC-RNDM 1.40 / CLB-TRANS 1.15 / DST-TRANS 1.15 / GND 0 · composite = log10-mapped MAX over (SX/SY/SZ driver-domain risk estimate) × DENS × REGIME × NAV × SURV per flight-hour then × phase-multiplier · hard escalators: pair-distance <3NM AND <500ft → composite-floor 90 / pair-distance <5NM AND <1000ft → 85 / two-or-more pairs both inside 5NM/1000ft window → 78 / OCEANIC-RNDM + RNP-10 + opposite-direction → 70 · 6 tiers mapped from log-scaled risk score: CRITICAL ≥85 rose MAC-rate >10× TLS (5×10⁻⁸/h) INVESTIGATE altimetry coordination or RNP navigation containment or longitudinal spacing / WATCH ≥65 rose-pink MAC-rate 1×-10× TLS monitor margin / MARGIN ≥45 amber MAC-rate 0.1×-1× TLS Doc 9689 SASP margin band / CLEAR ≥25 sky MAC-rate 0.01×-0.1× TLS / GOOD ≥10 emerald MAC-rate 0.001×-0.01× TLS / NEUTRAL <10 slate MAC-rate ≪ TLS isolated · TLS-foundation accident precedent library: Überlingen 2002 BTC2937 Tu-154 vs DHL611 B757-200F (BFU AX001-1-2/02 71 fatal · TCAS-RA vs ATC instruction conflict at FL360 NAT-EUR · drove TCAS 7.1 reversal logic + cross-coordination ATC training) / Cerritos 1986 PSA1771 / Piper PA-28 (NTSB AAR-87-07 87 fatal · pre-Mode-C VFR violation in KLAX TCA · drove §91.215 Mode-C veil + TCAS-II mandate) / Charkhi Dadri 1996 SVA763 / KZK1907 (AAIB India 349 fatal · head-on FL150 ATC-altitude misunderstanding · drove RVSM mandate acceleration in S Asia) / Gol 1907 / EMB Legacy 600 2006 (NTAB AT-DCA07RA005 154 fatal · opposite-direction same-level NAT corridor B737 vs ERJ-135 transponder coordination failure canonical RVSM TLS event) / São Paulo 1996 TAM RIO-SAO mid-air MD-11 vs F100 (drove DECEA RNAV-1 deployment in Brazil) / KLAX 2017 AC759 taxi/RWY approach (NTSB DCA17IA148 no fatalities · 4-aircraft taxiway near-miss · drove surface-CRM and runway-status-light deployment) / KIAH 2008 KMA UA-X CRJ vs DAL ERJ (TMA lateral-overlap RNP-1 conflict · contributed to NextGen PBN deployment timeline) / NAT 2017 BAW213 / DAL150 600ft excursion (NAT SPG Bull 2018-04 · MNPS deviation 4× TLS-target no fatalities) · MapLibre overlay tier-coloured outer halo radius=6+score·0.22 + CRITICAL/WATCH outer cascade ring + CRITICAL/WATCH/MARGIN pin + CRITICAL/WATCH cs+category+rate/h labels coloured by category swatch + DASHED line connecting worst-pair partners · AIRCRAFT/CATEGORIES/REGIMES/PRECEDENT/METHOD tab switcher · 6-tier counter strip click-to-filter ALL + 4-cell summary μ-Score / Worst-cs / Crit+Wat count / Σ-Rate/h vs TLS target + 4 sliders ADV-MUL 50-200% / REGIME 50-250% / MIN-FL 0-200 / MAX-FL 50-450 + 8-category chip filter V·RVSM/V·LARGE/L·RNP1/L·RNP4/L·RNP10/LX·TRAIL/OX·LONG/CLEAR + 7-phase chip filter (excl GND) + 8-class chip filter WB-RNP4/WB-RNP10/NB-RNP1/RGN-RNP1/BIZ-RNP4/TURBO-BRNAV/LIGHT-BASIC/OTHER + HALO/PIN/LBL/LINE/RING toggles + search by cs/type/category/phase · AIRCRAFT tier-worst-first row stack with SVG Reich-kernel sparkline (3-bar SX/SY/SZ axis breakdown vs TLS=50 dashed reference) + cs+type+class-pill+category-pill+phase-pill+tier-pill + FL/VS line + estimated-rate/h color-coded vs TLS / horizontal-NM-to-worst-pair / vertical-ft-to-worst-pair / OPP-DIR flag chip / neighbor-count + tier-coloured score bar + 7-driver chip grid SX/SY/SZ/DENS/REGIME/NAV/SURV + tier-coloured rationale advice citing investigated axis + worst-pair detail panel (other-cs/Δx/Δy/Δz km · Δv kt · Px/Py/Pz kernel values) · CATEGORIES per-category row with cat-coloured swatch + Doc 9689 description + crit+wat count + tier-coloured mean-score bar · REGIMES per-phase row with phase-multiplier coefficient + description + flight-count + μ-rate per phase · PRECEDENT TLS-foundation accident citations Überlingen / Cerritos / Charkhi-Dadri / Gol-1907 / TAM-SP / KLAX-AC759 / KIAH-2008 / NAT-2017 · METHOD full prose on Reich-Marks CRM formulation + ICAO TLS target + per-flight evaluator algorithm + phase classifier + hard escalators + distinctions from CPA/TCAS-RA/ACAS-X/STCA/MTCD/AIRPROX/CONFLICT/RVSM/RNP/ADSB-INT + full reference list · structurally distinct from CPA (geometric closest-point-of-approach pairing only — no probability, no airspace-planning context), TCAS-RA (downstream airborne resolution-advisory logic DO-185B — TLS is the design BASIS for separation standards that TCAS sits below as last-layer), ACAS-X (next-gen DP DO-385 collision-avoidance MDP — TLS is the framework against which ACAS-X is certified as not degrading the established TLS), STCA (controller-side Short-Term Conflict Alert threshold trigger — TLS is the probabilistic floor STCA defends), MTCD (Medium-Term Conflict Detection deterministic 8-30min trajectory probe — TLS is the longer-horizon design model), AIRPROX (post-event Risk Assessment Tool A/B/C/D/E severity classification of REPORTED encounters — TLS is FORWARD-LOOKING probabilistic risk-per-hour estimate), CONFLICT (binary detection ≤5NM 1000ft — TLS is the probabilistic FOUNDATION for that 5NM/1000ft threshold), DAA-WC (UAS Detect-And-Avoid Well-Clear RTCA DO-365B — separate UAS framework, conceptually distinct from CRM), RVSM (single-axis RVSM-equipment compliance — TLS integrates ALL three axes including RVSM as one driver), RNP / PBN (navspec route-keeping accuracy compliance — TLS combines RNP performance into the lateral overlap kernel as one driver), ADSB-INT (broadcast integrity NIC/NACp/SIL/SDA — TLS is the higher-level safety target that surveillance performance must support) — TLS is uniquely the AGGREGATED probabilistic pair-wise CRM evaluator scoring the estimated collision-risk-rate per flight-hour against ICAO Annex 11 Para 3.1.4 / Doc 9689 target 5×10⁻⁹ MAC/h: it is the PROBABILISTIC FOUNDATION for every deterministic separation overlay below it · (Reich PG 1966 "A Theory of Safe Separation Standards for Air Traffic Control" J.Inst.Navig 19 #1/#2/#3 / Marks BL 1963 RAE Tech Note 91 / ICAO Doc 9689 Manual on Airspace Planning Methodology for Determination of Separation Minima ed.2 1998 + Amend 2018 Chapter 2 CRM theory Chapter 3 TLS / ICAO Annex 11 Air Traffic Services Para 3.1.4 TLS / ICAO Doc 9574 Manual on RVSM Implementation 2002 / ICAO Doc 9613 PBN Manual ed.5 2024 / ICAO Doc 4444 PANS-ATM Chapter 5 Separation Methods / ICAO Doc 9854 Global ATM Operational Concept §3.6 Safety / ICAO Doc 9871 SARPS Mode-S extended squitter / ICAO SASP /29 WP-12 Reich CRM applied to PBN-RNP-1 TMA / ICAO SASP/30 WP-44 RNP-4 oceanic re-evaluation / ICAO RGCSP/9 WP-3 NAT MNPS TLS re-justification / NAT SPG Annual Reports 2010-2024 / RTCA DO-385 ACAS-X System Spec ARP-4754A safety case / FAA Order 7110.65 §5 Radar Separation / FAA AC 90-117 Data Link Communications / EUROCONTROL Specification on RNP-Approach / EUROCONTROL EATCHIP TLS validation 1996-2003 / Boeing D6-83820 NAT MNPS RVSM systems-cert / Brooker P 2004 "Air Traffic Separation: Reich and beyond" Aeronautical Journal 108 #1084 / Anderson D Lin Y 1996 "A Collision Risk Model for a Crossing Track Separation Methodology" J.Navig 49 #3 · TLS entry registered in Layers Safety & Traffic category after UPRT, ft-tls persisted preference)', showTls, ()=>{ const nv=!showTls; setShowTls(nv); lsSet('ft-tls', nv) }],
                ['VA-MAN · Maneuvering-Speed (V_A) Limit-Load & Cyclic-Rudder-Reversal Vertical-Stabiliser Departure Envelope Monitor · per-airframe live evaluator of each airborne aircraft′s instantaneous proximity to the §25.335(c) design maneuvering speed V_A (the maximum speed at which ABRUPT and MAXIMUM use of any ONE flight control from trim will not exceed the §25.337 limit load factor n_lim +2.5g/-1.0g §25 transport / +3.8g/-1.52g §23 normal) and the post-AA587 cyclic-rudder reversal load-amplification regime per 14 CFR Special FAR 109 (2008) / FAR §25.351 yaw-maneuver loads / AC 25.351-1 (2014) · post-AA587 critical lesson: §25.335(c) V_A protects ONLY against ABRUPT use of ONE control from trim — it does NOT protect against (a) cyclic full-rudder reversal (left-right-left, the AA587 N14053 sequence that yawed the A300-600 vertical stabiliser off the airframe at 250 KCAS / FL027 / 13:16:14 EST 2001-11-12 over Belle Harbor NY, 260+5 fatal per NTSB AAR-04-04 finding 4.2.B) (b) combined rolling-pulling-rudder inputs (AA191 DC-10 / FedEx-705 N306FE multi-axis pattern, exceeds §25.337 n_lim) (c) sustained sideslip beyond β_design (yaw structural envelope, addressed by SFAR-109 + AC 25-7D §5.4.2.2 + FAA SAFO 09010) · 9-class airframe maneuvering-envelope catalogue with per-class n_lim/V_A-typ/V_S-typ/β_design/SFAR-109 yaw-load reserve compiled from FCOM Vol 1 limits + FAA/EASA TCDS data: WB-HVY B777/B787/A350/A380 n_lim +2.5g V_A 282 V_S 175 β_des 7.5° yawRes 1.55× cycPen 1.65× M_MO 0.92 / WB-T2 B767/A330/A340/MD11 n_lim +2.5g V_A 270 V_S 165 β_des 7.0° yawRes 1.45× cycPen 1.75× / NB B737/A320/A321/A220/B757 n_lim +2.5g V_A 245 V_S 148 β_des 6.5° yawRes 1.40× cycPen 1.80× / RGN-J E170/E190/CRJ700/CRJ900 n_lim +2.5g V_A 220 V_S 138 β_des 6.0° yawRes 1.32× / RGN-T ATR42/72 Q400 n_lim +2.5g V_A 175 V_S 105 β_des 8.5° / BIZ G650/GLEX/Falcon/Citation n_lim +2.7g V_A 235 V_S 130 β_des 7.0° / MIL C-17/KC-135/C-130 n_lim +3.0g V_A 255 / LIGHT PC-12/TBM/Cirrus/C172 §23 n_lim +3.8g V_A 145 V_O 122 / OTHER · 8 driver scores [0..100] VA-MARG IAS proximity to weight-corrected V_A banded ramp / NORM-G |n-1| vs 0.85·n_lim normal-load proxy from VS swing × phase volatility / CYC-RUD cyclic-rudder reversal envelope penetration post-AA587 SFAR-109 / BETA sustained sideslip β vs class-specific β_design vertical-stab cert envelope / COMBO multi-axis full-deflection penalty roll+pitch+rudder simultaneous AA191/FedEx-705 mode / WT-CORR weight-correction inflation V_A = V_S·√n_lim heavier shifts V_A higher / TURB-AMP turbulence/wake/wind-shear amplifier of pilot input intensity / PHASE-W phase-weight CRZ 1.00 TKO-LIFT 1.20 GA 1.35 APPR 1.10 CLB 0.95 · composite max·0.66 + mean·0.34 × phase-W × ADV-MUL × turb-amp × 0.85 · hard escalators per AC 25.351-1 / SFAR-109 / AAR-04-04: CYC-RUD≥75 at IAS>0.95·V_A → 92 AA587 vert-stab departure mode / β≥1.4·β_des → 88 §25.351 + AC 25.351-1 envelope penetration / IAS>V_A+20 KIAS + |n-1|>0.7·(n_lim-1) → 82 §25.337(c) limit-load proximity / COMBO>60 + IAS>V_A → 75 AA191 / FedEx-705 combined-input envelope / CYC-RUD≥40 + IAS within 10 KIAS of V_A → 65 SFAR-109 cyclic case Airbus OEB-2002-49 Boeing FCTM 8.7 / |n-1|>0.8·(n_lim-1) in GA → 60 recovery bunting / β≥β_des in TKO-LIFT → 55 wake-encounter recovery · 6 tiers SHED ≥85 rose cyclic-reversal active at IAS>V_A OR β>1.5·β_des vert-stab departure imminent per AA587 N14053 precedent / LIMIT ≥65 rose-pink |n|>0.85·n_lim OR multi-axis full-deflection at IAS>0.9·V_A limit-load proximity / MARGIN ≥45 amber IAS approaching V_A within 20 KIAS with elevated maneuver intensity reduce to V_RA per AC 25.351-1 / GUARD ≥22 sky IAS within 40 KIAS of V_A in turb cruise monitor sideslip / CLEAR <22 emerald well below V_A normal envelope / OFF slate on-ground or below FL050 · accident precedent library: AA587 A300-600 N14053 2001-11-12 NTSB AAR-04-04 canonical cyclic-rudder reversal vert-stab departure 260+5 fatal · AAL903 A300-600 N90070 1997-05-12 W Palm Beach wake-recovery β=11° loaded 0.93 ultimate · UAL232 DC-10 N1819U 1989-07-19 Sioux City throttle-only post-UER §25.349 envelope · FedX705 DC-10 N306FE 1994-04-07 Memphis NTSB DCA94RA037 rolling-rudder hijack escape exceeded n_lim 2.5× · AF447 A330 F-GZCP 2009-06-01 Atlantic BEA F-CP090601 combined aft-stick + ALT-LAW §25.337 · TK1951 B737 TC-JGE 2009-02-25 AMS false-RA recovery aft yoke + bank · MapLibre overlay tier-coloured halo 7-19px by score + LIMIT/SHED rose pins + IAS≥45 labels with cs · V_A · Δ kt · β° + dashed yaw-departure vector top-20 worst aircraft length ∝ score · Side panel 6-tier counter strip click-to-filter ALL + 5-cell summary μ-Score / μ-VA-Δ / μ-β° / SHD+LMT count / CYC-ACT cyclic-active count + 2 sliders ADV-MUL 50-200% / VA-MUL 80-130% V_A calibration + 7-phase chip filter + 9-class chip filter + HALO/PIN/LBL/VEC toggles + search by callsign/type/operator + AIRCRAFT/CLASSES/VN-DIAG/METHOD tab switcher · AIRCRAFT tier-worst-first row stack with cs+type+class-pill+phase-pill+CYC-flag-chip+tier-pill + IAS/V_A/Δ-VA/V_MO 4-cell + n/β°/β-MRG/WT 4-cell + tier-coloured score bar + 8-driver chip grid VA-MARG/NORM-G/CYC-RUD/BETA/COMBO/WT-CORR/TURB-AMP/PHASE-W + tier-coloured advice citing investigated axis click-to-fly · CLASSES per-class row with class-coloured swatch + spec description + n_lim/V_S/V_A/V_MO/β_des 5-cell + yawRes/cycPen/μ-Δ/μ-β/μ-S 5-cell + SHD/LMT/MRG count + per-class mean-score gradient bar · VN-DIAG tab renders live V-n maneuver envelope diagram (V_TAS KIAS × n_z load factor) for selected aircraft (or worst) with CL_max stall boundary parabola positive+negative, +n_lim §25.337 limit-load horizontal line, -n_lim negative limit, V_A vertical V_RA vertical V_MO vertical reference lines, current aircraft state dot tier-coloured at (IAS, n) + 4-cell IAS/n/WT/cls readout + interpretation note + post-AA587 cyclic-rudder caveat block · METHOD tab full prose on scope + 8 drivers + composite + 7 hard escalators + 8 distinct-from analyses (GUST §25.341 / FLUTTER §25.629 / STALL α-floor / COFFIN Mach-Vs1g / UPRT AUPRTA aggregated / VMC asymmetric-thrust / MACH-TUCK transonic / DUTCH-ROLL eigenmode) + 6 accident precedents + full references list · structurally distinct from GUST (vertical-gust §25.341 discrete Pratt-Walker load case — VA-MAN is MANEUVER §25.337(c) load case, different cert path), FLUTTER (aeroelastic V_f §25.629 eigenmode — VA-MAN is rigid-body), STALL (low-α α-floor — VA-MAN is load-factor envelope at any α), COFFIN (Mach-Vs1g cruise convergence — VA-MAN is maneuver envelope all phases), UPRT (aggregated AUPRTA Rev 3 — VA-MAN is the single-axis MANEUVER speed margin driver that feeds UPRT G-LOAD score), VMC (lateral-directional asymmetric-thrust floor — VA-MAN does not require OEI), MACH-TUCK (transonic Cm(M) reversal — VA-MAN is subsonic maneuver envelope), DUTCH-ROLL (lateral-directional eigenmode damping — VA-MAN is rigid-body load envelope at rudder+elevator authority floor), MCAS (B737MAX-specific stab-trim runaway — VA-MAN is pilot-input load-amplification envelope), TRIM (mistrim authority residual — VA-MAN is full-deflection from trim envelope), CG-TRIM (longitudinal CG envelope — VA-MAN is speed envelope coupled to weight via V_A = V_S·√n_lim) — VA-MAN is uniquely the §25.335(c)/§25.337/§25.351 RIGID-BODY MANEUVER-LOAD envelope evaluator scoring IAS-to-V_A proximity, cyclic-rudder reversal envelope penetration risk (post-AA587 SFAR-109/AC 25.351-1 §6), combined roll-pitch-rudder input amplification, sustained sideslip β > β_design envelope, and speed regime classification VS-bnd / VA-mrgn / VC-OK / VC-VMO / VMO-VD / VD+ · References 14 CFR §25.143/.147/.301/.303/.305/.307/.335/.337/.341/.349/.351 / §23.335/.337/.351 · EASA CS-25.143/.335/.337/.351 Amdt 15 2014 · FAA AC 25-7D §5.4 high-speed handling / AC 25.351-1 (2014) Yaw Maneuver Conditions / SAFO 09010 (2009) Rudder Use Guidance / Special FAR 109 Final Rule 2008-09-25 / InFO 11015 Use of Rudder · ICAO Annex 6 Pt I §4.3 / Doc 9760 Vol II Pt IV §4 / Doc 10011 (UPRT) §2 envelope definitions / IATA APS UPRT 2nd ed (2019) §3.4 rudder use · Boeing FCTM 8.7 Adverse Weather + FCOM SP.16.6 · Airbus FCOM PRO-NOR-SOP-13 + DSC-27 FBW yaw-protection + AAL TBC OEB-2002-49 post-AA587 OEM bulletin · Embraer AOM §03 · CRJ FCOM Vol 1 §03 · ATR FCOM Vol 1 §03 · NTSB AAR-04-04 (AA587 canonical) · NTSB DCA94RA037 (FedEx-705 multi-axis envelope) · BEA F-CP090601 (AF447 combined input) · ATSB AO-2008-070 (QF72 ADIRU upset) · Roskam Flight Dynamics Pt I §3 maneuver loads · Etkin & Reid Dynamics of Flight 3e §7 maneuver envelope · Anderson AP&D 6e §6 V-n diagram · Hoblit Gust Loads AIAA 1988 · Roskam Airplane Design Pt V §6 maneuver loads · VA-MAN entry registered in Layers Safety & Traffic category after TLS, ft-vaman persisted preference', showVaman, ()=>{ const nv=!showVaman; setShowVaman(nv); lsSet('ft-vaman', nv) }],
              ]},
              ...
              {group:'Analysis', items:[
                ...
                ['SCRM · Sterile-Cockpit-Rule (§121.542 / §135.100) & Critical-Phase-of-Flight Crew-Distraction Vulnerability Monitor · per-airframe live evaluator of every aircraft currently inside a critical phase of flight in which the 14 CFR §121.542 / §135.100 Flight Crewmember Duties prohibition on non-essential cockpit activity is in force (taxi, takeoff, landing, and all flight operations conducted below 10,000 ft MSL except cruise), and the §121.585 / §135.144 cockpit Personal Electronic Device prohibition that augments it — scoring each cockpit\'s vulnerability to crew-distraction-induced loss-of-situational-awareness across an 8-driver decomposition: PHASE (sterile-zone in force × per-phase weight) / PED-RISK (§121.585 cockpit PED-use proxy ~3-8% violation band) / COMM-LOAD (R/T frequency congestion / ATC tower-talkdown saturation per FAA Order JO 7110.65 §2-4-21 frequency hygiene) / CHECKLIST (checklist-window overlap with conflict / abnormal input per FSF ALAR BN 2.4) / CABIN (PA / cabin-call / jumpseat-occupied / FA chime rate per IATA OPS-Brief 2024) / FMS-REPRO (late FMS reprogramming overlap with TOD or below-10kft window per AC 120-71B) / ABNORMAL (squawk 7500/7600/7700, emergency flag, abnormal VS/IAS deviation) / TASK-SAT (composite saturation index: ≥3 simultaneous active threads collapses crew bandwidth) · structurally distinct from CIRCAD (multi-day circadian Borbély-Achermann fatigue accumulation across the FDP — SCRM is the IN-PHASE acute distraction snapshot), WORKLOAD-TLX (NASA TLX subjective workload index — SCRM is the regulatory-compliance scorer for the §121.542 PROHIBITION not the workload demand magnitude), CREW-DUTY (FAR 117 FDP duration / rest cap — SCRM is duty-IN-PHASE not duration), FMA (cockpit autoflight mode-annunciator subsystem state — SCRM is crew-distraction-vulnerability not mode awareness), PIO (closed-loop pilot bandwidth handling-quality — SCRM is regulatory-prohibition compliance not control-loop dynamics), CSC (call-sign confusion R/T mis-identification — SCRM is the broader crew-discipline regime that includes CSC as a COMM-LOAD sub-factor), STABLE-APP (1000ft/500ft stabilised-approach gate gross-checks — SCRM is the underlying sterile-cockpit ATTENTION REGIME that protects the stabilisation), CRM (Crew Resource Management training framework — SCRM is the LIVE compliance scorer that CRM trains for) — SCRM is uniquely the §121.542 / §135.100 / §121.585 / §135.144 sterile-cockpit-rule compliance evaluator asking (a) is the airframe in a sterile-zone phase per AC 120-71B §7.3 classifier (b) is any cockpit distraction signature active (c) is the composite distraction score at BREACH/CRITICAL/ELEVATED level requiring crew intervention · 9-phase classifier auto-classified from kinematics: TAXI (ground 5-30kt) / TKOFF-ROLL (>30kt accelerating) / INIT-CLB (airborne <10,000ft AGL climbing per §121.542 below-10kft rule) / TOD-DSC (FL110-FL140 descending entering sterile zone) / INIT-APP (<10,000ft MSL inbound) / FNL-APP (<3,000ft AGL flare/final) / LDG-ROLL (>30kt decelerating ground) / POST-LDG (<30kt post-touchdown sterile until clear of active runway holding positions) / GND-STOP (stationary <5kt at gate/hold-short) plus CRZ-CLEAN baseline (NOT sterile per §121.542 above-10kft exemption) — phase weights peak FNL-APP ×1.60 TKOFF-ROLL ×1.55 LDG-ROLL ×1.45 INIT-CLB ×1.40 INIT-APP ×1.35 TAXI ×1.30 POST-LDG ×1.20 TOD-DSC ×1.05 GND-STOP ×0.20 CRZ-CLEAN ×0.05 · per-operator crew-discipline floor classifier HIGH (military Part 121 major-carrier business-jet NBAA-trained ×0.82) / MID (LCC ×1.00 baseline) / LOW (small Part 135 charter ×1.18) · 8-driver scoring composite max·0.66 + mean·0.34 × ADV-MUL × discipline-floor · hard escalators ABNORMAL≥90 in sterile-zone score-min 88 / PED-RISK≥90 in critical phase score-min 86 (§121.585 BREACH) / ≥5 active drivers ≥40 score-min 78 (task-saturation collapse per CO 2574 EMB-120 precedent) · 6 tiers BREACH ≥85 rose §121.542 / §121.585 explicit breach immediately silence non-essential intercom + secure PEDs + brief PM "we are below ten thousand" per AC 120-71B §7 / CRITICAL 70-84 rose-pink active distraction signature verbalise interruption "STERILE COCKPIT" + transfer non-PF tasks per FSF ALAR BN 2.4 / ELEVATED 55-69 amber multi-driver elevated risk brief crew next handoff per AC 120-48A / WATCH 35-54 sky sterile-zone present standard SOP discipline / NOMINAL <35 emerald sterile-zone managed normal CRM holding per ICAO Annex 6 Pt I §4.3.2.4 / OFF slate outside sterile window above-10kft cruise exemption · 14 precedent cases catalogued in PRECEDENTS tab: Eastern 401 KMIA 1972-12-29 L-1011 Everglades CFIT (101 fatal, the THE accident that DROVE §121.542 adoption in 1981, captain distracted by burnt-out gear-position bulb let AP disconnect undetected — NTSB AAR-73-14) / Comair 5191 KLEX 2006-08-27 CRJ-100 wrong-runway takeoff RW26 instead of RW22 (49 fatal, finding F-4 non-pertinent crew conversation during taxi BREACH §121.542, THE modern sterile-cockpit case — NTSB AAR-07-05) / Colgan 3407 KBUF 2009-02-12 DH8D (50 fatal, finding F-12 non-pertinent conversation below 10,000ft + stick-pusher mis-response — NTSB AAR-10-01) / Continental Express 2574 KEGT 1991-09-11 EMB-120 in-flight breakup (14 fatal, horizontal-stab leading-edge attach-screws missing undetected during distracted descent, task-saturation finding — NTSB AAR-92-04) / Continental 1943 KIAH 1996-02-19 B737-300 gear-up landing (0 fatal, distracted-approach missed gear-extension checklist item) / Asiana 214 KSFO 2013-07-06 B777-200ER (3 fatal, unstable approach distracted A/T mode awareness — NTSB AAR-14-01) / BA 5390 1990-06-10 BAC1-11 (0 fatal, captain partially ejected, windshield-bolt mis-replacement contributed by checklist-window distraction — AAIB EW/C1095) / DHL 611 / TU154M Überlingen 2002-07-01 mid-air (71 fatal, controller distracted by simultaneous radar workstation tasks — BFU AX001-1-2/02) / AA 1420 KLIT 1999-06-01 MD-82 thunderstorm runway overrun (11 fatal, captain task-saturated spoiler-arming missed — NTSB AAR-01-02) / SW 3472 2016-08-27 B737-700 engine FBO FL311 distraction-MANAGED by trained sterile-discipline crew (0 fatal, positive case — NTSB DCA16IA218) / UAL 232 KSUX 1989-07-19 DC-10 uncontained tail-engine all-hydraulics-lost (111 fatal but exemplary CRM under task-sat — NTSB AAR-90-06 positive case) / AF 447 2009-06-01 A332 (228 fatal, captain absent off-rest, pitot-icing AL2 reversion confusion — BEA F-PR090601) / Alaska 1282 KPDX 2024-01-05 B737-9 door-plug blowout FL150 (0 fatal, crew managed under sterile-discipline emergency-descent — NTSB DCA24MA063) / China Airlines 140 Nagoya 1994-04-26 A300B4-600R (264 fatal, TO/GA mode inadvertent latched copilot mode confusion deep-stall — AAIC CAL 140 1996) · MapLibre overlay tier-coloured halo rings 6-26px sized by composite score + BREACH/CRITICAL rose pins + dashed forward-track sterile-zone 3NM-cone projection (INIT-CLB / INIT-APP / FNL-APP) + cs+phase+tier labels · Side panel 6-tier counter strip + 5-cell summary μ-SCR / BREACH-cnt / CRIT-cnt / FNL-cnt / TXY-cnt + 3 sliders ADV-MUL 50-200% / PED-MUL 50-200% (§121.585 enforcement scaler) / COMM-MUL 50-200% (R/T congestion scaler) + 4-class crew-discipline filter HIGH/MID/LOW + HALO/PIN/LBL/CONE toggles + 9-phase chip filter + search by callsign/type/operator/phase + AIRCRAFT/PHASES/PRECEDENTS/METHOD tab switcher · AIRCRAFT tier-sorted worst-first row stack with cs+type+tier-pill+phase-pill+discipline-pill + AGL/GS/VS/SQ/FL/TOP-driver/OPER/SCR 8-cell + tier-coloured score bar + 8-driver severity-coloured chip strip + tier-coloured advice line citing regulatory ref · PHASES per-phase row showing phase-weight × multiplier + N/BREACH/CRIT/FMS-risk/CABIN-rate 5-cell + ⌀-SCR pill + italic phase-citation footnote · PRECEDENTS 14-case scrollable cards with id/date/fatal/phase pill + trigger + finding + citation · METHOD tab regulatory basis + phase classifier explainer + 8-driver scoring decomposition + hard-escalator listing + 6-tier definitions + 14-precedent index + full reference chain · (14 CFR §121.542 · §135.100 · §121.585 · §135.144 · ICAO Annex 6 Pt I §4.3.2.4 · Doc 9966 FRMS Manual App.B · FAA AC 120-71B SOPs Ch.7 Sterile-Cockpit / Critical-Phase SOP · AC 120-48A Communication & Coordination Crew Discipline · FAA Order JO 7110.65 §2-4-21 ATC Frequency Hygiene · FSF ALAR Briefing Note 2.4 Interruptions & Distractions · FSF Flight Safety Digest Jul-Sep 2003 Sterile-Cockpit-Rule survey · IATA Safety Report 2024 §3.4 Loss-of-State CFIT distraction · NTSB AAR-07-05 Comair 5191 · NTSB AAR-73-14 Eastern 401 · NTSB AAR-92-04 Continental Express 2574 · NTSB AAR-96-03 Continental 1943 · NTSB AAR-10-01 Colgan 3407 · NTSB AAR-14-01 Asiana 214 · NTSB SIR-94-01 Sterile-Cockpit-Rule Compliance Special Investigation · AAIB EW/C1095 BA 5390 · BFU AX001-1-2/02 Überlingen)', showScrm, ()=>{ const nv=!showScrm; setShowScrm(nv); lsSet('ft-scrm', nv) }],
              ...
              {group:'Routes & Flow', items:[
                ...
                ['GASA · Go-Around / Baulked-Landing Initiation & Energy-Margin Monitor · per-airframe live evaluator of every aircraft that has actively initiated (or sits at materially elevated probability of initiating) a GO-AROUND / BAULKED-LANDING from short-final, flare or post-touchdown — scoring the energy state captured DURING the manoeuvre itself rather than the published-procedure capability or the pre-touchdown gate criteria · structurally distinct from MISSED-APPROACH (the OEI net-climb-gradient CAPABILITY evaluator for the PUBLISHED missed-approach segment per PANS-OPS Doc 8168 Vol II Pt I §4.6 — a CAPABILITY metric computed ahead of time for every approaching aircraft, NOT an active GA-detection scorer), STABLE-APPROACH (the 1000 ft / 500 ft gate-criteria evaluator for FSF ALAR / EASA AMC 121.1A — what TRIGGERS a go-around decision, NOT what happens after one starts), TEM-ENERGY (the global kinetic+potential trade-off budget — used at all phases not GA-specific), CDFA/VDP (continuous-descent final approach conformance — also a pre-touchdown vertical-path metric), BTV/ROW-ROP (Brake-To-Vacate exit selection + Runway Overrun Warning/Prevention — landing rollout after a LANDING not a GA-initiation event), BOUNCE-TOUCHDOWN (touchdown dynamics scorer — a different outcome of the same approach where landing IS completed), MA-OEI (Missed-Approach OEI 2nd-segment — capability), BFL/RTOW (Balanced Field Length / RTO Weight — only for departure) — GASA is uniquely the ACTIVELY-IN-PROGRESS go-around event monitor that flags an airframe as IN-GA when its track shows (a) at-or-below 1500ft AGL within 5NM final-approach capture-box of an aligned destination, (b) reversal of vertical-rate sign (VS swings from DSC to CLB at <1500ft AGL within last 60s OR currently VS>+700fpm at <2500ft AGL with GS>130kt), (c) thrust-elevation proxy (estimated N1 jump >18% from approach-idle to GA-thrust per FCOM PI-11) and then computes the live GA energy-margin score from TOGA-N1 thrust application timing vs published TOGA latch criteria (Boeing FCOM CL go-around call → TOGA switches forward / Airbus FCOM PRO-NOR-SOP-22 thrust levers → TOGA detent → A/THR wakeup → FMA GOAROUND IDLE/PITCH 2/SRS armed) + pitch-attitude vs class tailstrike limit (B777 12.5° / B738 11° / A320 13.5° / A380 9.0° / E190 12° per FCOM Limits Ch.1 + FCTM 6.20 tail-strike avoidance with deeper tabulation for 7 fleet classes) + gear-retraction altitude/positive-climb sequence (POSITIVE-RATE / GEAR-UP call expected at +50fpm sustained AGL > 200ft per FCOM CL 4-step go-around gear-up before VLO 235 KIAS) + flap-retraction schedule (Boeing 737 F15→F5 at V2+15 → F1 at V2+30 → F0 at clean-up speed VFTO per FCOM PI-11.40 / Airbus F3→F2 at S+10 → F1 at F-speed → F0 at green-dot speed per FCOM PRO-NOR-SOP-22) + OEI net 2nd-segment gradient ≥2.4% per CS-25.121(b) hot-and-high adjusted for measured weight/OAT/PA + pitch-rate command vs α-PROT margin (Airbus Normal-Law GA captures pitch via SRS-GA mode + α-PROT shield / Boeing FBW pitch-by-flight-path angle) + lateral track conformance to published MAP (±1.5 NM RNP-AR tolerance per Doc 9613 Vol II Pt C) + re-sequencing decision (proceed to MAP holding fix per Doc 4444 §6.5 vs vectors-for-resequence per JO 7110.65 §5-8/§5-9) · canonical accident/incident precedent Asiana 214 OZ214 2013-07-06 KSFO 28L (NTSB AAR-14-01) 777-200ER HL7742 attempted late-GA after low-energy short-final undershoot F-PLD TOGA actuation lagged impact 28L seawall 3 fatalities A/THR NOT in SPEED mode (HOLD) proximate cause / Atlas Air 3591 GTI3591 2019-02-23 KIAH ILS 25R (NTSB AAR-20-02) 767-300F N1217A inadvertent GA-pitch-mode actuation (TO/GA detent inadvertently triggered via go-around switch on yoke) pilot-induced pitch-down somatogravic over Trinity Bay impact 3 fatalities / Continental 1404 CO1404 2008-12-20 KDEN 34R (NTSB AAR-10-04) B737-500 abort-after-touchdown rejected-landing/partial GA decision-confusion in crosswind runway excursion 38 injured hull loss / EVA 015 BR015 2017-12-29 KLAX 06R (NTSB CEN18LA088) 777-300ER low-altitude GA over Compton CA at <500ft AGL with 6-NM track deviation neighbourhood overflight at <300ft AGL FAA enforcement action / Air India Express 1344 IX1344 2020-08-07 VOCL Calicut (AAIB IND-AAIB-AR-19/2020) 737-800 VT-AXH delayed GA decision on tailwind wet-runway landing overrun table-top runway 10 21 fatalities / Lion Air 904 JT904 2013-04-13 WADD Denpasar 738 PK-LKS late-GA-go missed touchdown over sea overshoot all 108 survived water ditching short of runway 09 threshold · 7-class GA-energy catalogue HVY-T togaN1=105 tsLimit=12.5° γ2-min=2.4% vRef=148kt B777/B787/A330/A350 FCOM PI-11.20 / HVY-Q togaN1=104 tsLimit=9.0° γ2-min=2.4% B747-8/A380 (tailstrike-critical) / WB-M togaN1=105 tsLimit=11.5° γ2-min=2.4% B767/A330ceo Asiana 214 precedent class / NB togaN1=102 tsLimit=11.0° γ2-min=2.4% B737NG/B737MAX/A320 IX1344+CO1404 precedent / RGN-J togaN1=101 tsLimit=11.5° E190/E2/CRJ / RGN-T togaN1=98 tsLimit=13.0° AT72/Q400/Saab / BIZ togaN1=103 tsLimit=12.0° G650/GLEX/FA8X · 8 drivers each 0-100 TOGA thrust-application timing (lag past GO-AROUND call → TOGA detent → A/THR wakeup → FMA GA mode latched) / PITCH pitch-attitude vs tailstrike margin / GEAR gear-retraction sequencing (POSITIVE-RATE call → gear-up before VLO 235 KIAS) / FLAP flap-retraction schedule conformance vs per-class FCOM cleanup table / GRAD OEI 2nd-segment net gradient vs 2.4% certificated floor per CS-25.121(b) / ALPHA high-AOA shield approach (α-PROT margin Normal-Law / stick-shaker margin Boeing) / TRACK lateral conformance to published MAP track tolerance (±1.5 NM RNP-AR) / SOMAT somatogravic-pitch-down risk (high-thrust acceleration vector vs visual horizon — Atlas 3591 precedent over dark water/IMC) · composite max·0.22·PITCH + 0.20·GRAD + 0.14·TOGA + 0.12·ALPHA + 0.10·SOMAT + 0.08·GEAR + 0.08·FLAP + 0.06·TRACK × phase-weight × ADV-MUL · hard escalators pitch-margin<1.0° in TOGA-LATCH/INI-CLB score-min 92 (TAILSTRIKE imminent per FCTM 6.20) / γ2 <floor in TOGA/INI/FLAP-RETR min 86 (request hot-and-high vectors) / TOGA-lag > ceiling in TOGA-LATCH min 82 (Asiana 214 mode-confusion class verify TOGA detent + FMA GA SRS+THR latched) / α-PROT margin <25 in TOGA/PRE-GA min 76 (low-energy GA capture watch stick-shaker) / somatogravic >75 in TOGA/INI-CLB min 68 (Atlas 3591 precedent trust ADI not seat-of-pants) / gear-up >1500ft AGL in INI-CLB min 58 (drag penalty erodes 2nd-seg γ ≥0.4%) / track-err >2NM in INI/FLAP min 54 (RNP-AR MAP bust verify NAV mode + correct MAP loaded) / flap-cleanup <50% in FLAP-RETR min 45 · 6 tiers CRITICAL ≥85 rose catastrophic GA-mismanagement tailstrike or terrain impact imminent (Asiana 214 / Atlas 3591 / IX1344 mode) / POOR ≥65 rose-pink unsafe GA execution (low altitude gradient bust late TOGA α-PROT shield breach) / WATCH ≥45 amber marginal GA execution (mode-confusion proxy flap-cleanup behind schedule somatogravic vulnerability) / NOMINAL ≥22 sky standard GA per FCOM (TOGA latched pitch within tailstrike margin γ ≥2.4% with ≥0.5% margin) / CLEAN <22 emerald textbook GA execution (γ ≥3.5% all four FMA conditions confirmed) / OFF slate not in GA window / on ground / cruise · MapLibre overlay halo ring sized by score 8-22px tier-coloured + CRITICAL/POOR rose pin + dashed tier-coloured GA-vector projection from current position along track to predicted 3500ft AGL crossing + tier-coloured label cs/phase/pitch°/gradient% · 4 sliders ADV-MUL / TS-MARGIN 1-6° (tailstrike margin floor) / GRAD-FLOOR 2.0-4.0% / TOGA-LAG-MAX 3-10s / GA-CAPTURE-NM 3-12 · 7-phase chip filter PRE-GA/TOGA-LATCH/INI-CLB/FLAP-RETR/CLEAN-UP/MAP-HOLD · 7-class chip filter · HALO/PIN/LBL/VEC/RWY toggles · 4-tab AIRCRAFT/CLASSES/DRIVERS/PROCEDURE with SVG GA-flight-path diagram showing canonical TOGA→POSITIVE-RATE→GEAR-UP→flap-retract→MAP-hold milestones plus live IN-GA aircraft samples · references FCOM PI-11.20 / FCTM 6.20 / CS-25.121(b) / PANS-OPS Doc 8168 Vol II Pt I §4.6 / Doc 9613 Vol II Pt C RNP-AR / Doc 4444 §6.5 / FAA JO 7110.65 §5-8/§5-9 / NTSB AAR-14-01 Asiana 214 / NTSB AAR-20-02 Atlas Air 3591 / NTSB AAR-10-04 Continental 1404 / AAIB IND-2020 Air India Express 1344 / NTSB CEN18LA088 EVA 015 / KNKT Lion Air 904 · GASA entry registered in Layers Routes & Flow category between Stable approach and Approach mins (active GA-event monitor sits next to its causal trigger), ft-gasa persisted preference', showGasa, ()=>{ const nv=!showGasa; setShowGasa(nv); lsSet('ft-gasa', nv) }],
                ...
                ['APCH-CAT · ILS CAT-I/II/IIIA/IIIB Low-Visibility-Operations Approach-Category Compliance & Equipment-vs-Weather Eligibility Monitor · per-airframe live evaluator of every aircraft on final approach to one of 24 catalogued ILS-equipped hub runways (KJFK/KEWR/KBOS/KATL/KORD/KMIA/KLAX/KSFO/KSEA/KDEN/CYYZ/EGLL/EHAM/EDDF/EDDM/LFPG/LSZH/LIRF/OMDB/WSSS/VHHH/RJTT) deciding whether the combination of (a) reported weather (RVR/vertical-vis/cloud-base ceiling), (b) aircraft-class autoland/HUD-EVS equipment fit (FailOp channels), (c) ground-equipment integrity (LLZ+GP transmitter status, RVR-sensor readiness, LVP runway guard-rail / sensitive-area protection), and (d) crew currency/type-rating proxy, satisfies the certified ILS approach-category minima per 14 CFR §91.175 §91.189 §121.651 §121.652 / ICAO Annex 6 Pt I §4.2.8 / Annex 10 Vol I §3.1.3 ILS course-tolerance / ICAO Doc 8168 PANS-OPS Vol I §1.4 CAT-I/II/III minima / ICAO Doc 9365 All-Weather Operations Manual ed.3 / FAA AC 120-28D CAT-IIIb FailOp no-DH ops / FAA AC 120-29A CAT-I/II approval / FAA AC 90-106A EFVS-to-landing / EASA CS-AWO All-Weather Operations / AMC1 CAT.IDE.A.105 / EASA Decision 2019/021/R AWO ed.2 / FAA Order JO 7110.65 §3-1-7 LVPs RVR≤1200ft / FAA Order 8400.13D CAT-II/III ops approval · CAT-band envelope CAT-I DH≥200ft RVR≥550m / CAT-II 100≤DH<200 RVR≥300m / CAT-IIIA 0<DH<100 RVR≥175m / CAT-IIIB no-DH or DH<50 RVR≥50m / CAT-IIIC no-DH no-RVR not yet authorised · 10-class equipment fit HVY-T-NEW B777/B787/A350/A330neo 3-AP FailOp no-DH CAT-IIIB / HVY-Q B747-8/A380 3-AP FailOp / WB-M B767/A330ceo 2-AP FailOp CAT-IIIA DH50 / NB-NEW B737NG/B737MAX/A320neo/A321XLR CAT-IIIA / NB-CLS B737CL/B757/A320ceo CAT-IIIA DH50 limited / RGN-J-NEW E190/E2/CRJ900/CRJ1000 CAT-II DH100 / RGN-T AT72/Q400/Saab CAT-I DH200 / BIZ-HUD G650/GLEX/FA8X HUD-EVS CAT-II equiv credit / BIZ-STD PC12/C25B CAT-I / LIGHT visual-only · per Boeing FCOM Limits Ch.1 / Airbus FCOM LIM-22 / Embraer E1/E2 AFM §2 / ATR-72/Q400 FCOM §2.04 / Gulfstream/Bombardier biz-jet AFM §2 · weather synthesis per ICAO Annex 3 RVR-vis-ceil bands DOY-cycling fog-frequency seasonal-winter cos² mid+high-lat fog surge / lat-band polar+mid-lat+tropical / inverse-frequency table 18% IIIB / 30% IIIA / 50% II / 75% I-marginal / 100% VMC · runway-snap great-circle ≤SCOPE-NM with track-vs-ldgHdg alignment ≤ALIGN-DEG + bearing-to-threshold ≤35° · LVPs in-force per JO 7110.65 §3-1-7 when RVR≤365m or ceil≤200ft · 6 drivers WX-CAT weather worse than aircraft/runway can support / EQUIP airframe below cat required by WX / CREW currency/recency proxy / GAFOR ground-equip LLZ/GP/RVR sensor degraded / REDUND FailOp redundancy lost (1-AP only when 2-AP required) / APPR-BAN CAT-III sensitive-area not protected · composite max·0.62+mean·0.38 × ADV-MUL · hard escalators EQUIP<WX-required score≥80 NOGO / ground-fault>0.85 score≥70 / APPR-BAN≥60 score≥70 · 6 tiers NOGO≥80 rose WX requires above airframe ceiling divert per AC 120-28D §3 / 14 CFR §91.175(c) / DIVERT≥60 rose-pink ground-equip fault or CAT-III BAN holding/alt diversion per CAT.OP.MPA.110 / DOWNGRADE≥40 amber accept higher DH/RVR per FCOM Lim Ch.1 / WATCH≥22 sky LVPs in force monitor approach / CLEARED<22 emerald within minima per JO 7110.65 §3-1-7 / ABOVE-MINS slate WX permits visual approach all CAT bands available · MapLibre overlay tier-coloured halo+pin+dashed link aircraft→runway + runway markers labelled ICAO+rwy+cat · Side panel 6-tier counter strip click-to-filter ALL + 5-cell summary In-Apch/LVPs-ON/CAT-III-cnt/μ-Score/Rwys-cnt + 4 sliders ADV-MUL/FOG-MUL/DOY/SCOPE-NM + 10-class chip filter + 7-region chip filter (NAM-E/NAM-W/NAM-S/EUR/ME/ASIA-S/ASIA-E) + HALO/PIN/LBL/RWY/LNK toggles + search by callsign/type/icao/cat + AIRCRAFT/RUNWAYS/EQUIPMENT/METHOD tabs · AIRCRAFT tier-sorted row stack with cs+type+class-pill+tier-pill + rwy/cat/DH/RVR/ceil/FailOp/HUD-EVS/LVP/dist 8-cell + tier-coloured score bar + driver chips + tier-coloured advice citing regulatory ref · RUNWAYS per-rwy active-traffic with worst-tier-pill + pub-CAT/use-CAT/RVR/ceil/cnt 5-cell + LLZ/GP/elev italic + LVP banner · EQUIPMENT per-class catalogue with capCat/FailOp/HUD-EVS chips + exemplars + count/μ-score + DH/RVR minima · structurally distinct from STABLE-APPROACH (1000ft/500ft gate gross check not category), CDFA-VDP (NPA vertical-path conformance not precision-cat), APPR-MINS (publishes the minima APCH-CAT scores compliance), STBR (stop-bar enforcement on ground), VFE (flap-speed envelope), DECRAB (crosswind technique), ROW-ROP (rollout overrun) — APCH-CAT is the All-Weather-Operations regulatory-eligibility scorer answering the operational question "is this airframe RIGHT NOW certified AND equipped AND crewed AND has the ground-equipment integrity to legally continue this CAT-IIIb approach in the reported 75m RVR?"', showApchCat, ()=>{ const nv=!showApchCat; setShowApchCat(nv); lsSet('ft-apchcat', nv) }],
                ['CDA compliance', showCda, ()=>{ const nv=!showCda; setShowCda(nv); lsSet('ft-cda', nv) }],
                ['CVFP · Charted Visual Flight Procedure Compliance & Landmark-Track Conformance Monitor · per-airframe live evaluator of every aircraft currently inbound to one of 18 catalogued FAA-published CHARTED VISUAL FLIGHT PROCEDURE (CVFP) approaches — the visual-only approach charts that overlay specific terrain landmarks, river bends, bridges and highway alignments as the primary pilot reference in lieu of instrument guidance (RIVER VISUAL 19 DCA · EXPRESSWAY VISUAL 13 LGA · CANARSIE VISUAL 13L/R JFK · PARKWAY VISUAL 22 SFO · QUIET BRIDGE VISUAL 28L SFO · FMS BRIDGE VISUAL 28R SFO · TIPP TOP VISUAL 30R STL · CHARLES RIVER VISUAL 33L BOS · HARBOR VISUAL 4R BOS · TIBURON VISUAL 28L SFO · BAY VISUAL 10L SFO · STADIUM VISUAL 31C MDW · PRESIDENT VISUAL 4R MDW · COLISEUM VISUAL 25R LAX · MEMORIAL VISUAL 27R PHL · STADIUM VISUAL 12R MSP · WATERFRONT VISUAL 15L BWI · CAPITAL VISUAL 1R IAD) scoring whether each inbound airframe (a) is INSIDE the published CVFP corridor envelope (track / altitude / dist / WX minima per AIM 5-4-23), (b) is HITTING each charted landmark waypoint at the published altitude (RIVER VISUAL DCA: Key Br 1500 / Memorial Br 1100 / Pentagon 700 / 14th-St Br 450 / River-Bend 300ft, etc.), (c) has the REQUIRED VMC (ceiling ≥ 3500ft AGL, visibility ≥ 3sm per 14 CFR §91.155 Class B/C/D), and (d) is COMPLYING with airframe-category / weight / noise restrictions inherent to the procedure (EXPRESSWAY VISUAL LGA-13 limited to Group I jets ≤ 100,000 lbs per JO 7110.65 §7-4-5b · QUIET BRIDGE SFO-28L restricted to Stage-4 airframes per LSFNR · CANARSIE VISUAL JFK-13L/R restricted to crews holding OpSpec C062 · TIBURON VISUAL SFO-28L Stage-4 + noise) per 14 CFR §91.175(j) operation below DA/DH/MDA / §91.155 VFR weather minimums / §91.131 ATC auth Class B / §91.129 Class D / FAA AIM 5-4-23 Charted Visual Flight Procedures CVFP / AIM 5-4-22 Side-Step Maneuver / AIM 5-4-20 Approach Lighting Systems / AIM 5-4-7 Approach categories / FAA Order 8260.3D TERPS Vol I §232 CVFP construction / FAA Order 8260.19J Flight Procedures Airspace / FAA Order JO 7110.65 §7-4 Visual Approach procedures / §7-4-5 CVFP issuance / ICAO Doc 8168 PANS-OPS Vol II Pt I §3.1 Visual manoeuvring / Annex 6 Pt I §4.2.8.2 Visual reference required / OpSpec C062 Special CVFP authorisations (CANARSIE) / OpSpec C063 / OpSpec C075 IFR-to-Visual transition / AC 90-114B ADS-B & visual separation / AC 91-79B mitigating runway overrun · 7-class fleet catalogue HVY-T Cat-D Vref148kt (B772/B77W/B788/B789/B78X/A359/A35K/A332/A333/A338/A339/B744/B748/A388 — non-GroupI Stage-4) / NB Cat-C Vref135kt (B737/B738/B739/B38M/B39M/A319/A320/A321/A20N/A21N/BCS3/B752 — GroupI Stage-4) / RGN-J Cat-C Vref128kt (E170/E175/E190/E195/E290/E295/CRJ7/CRJ9 — GroupI Stage-4) / RGN-T Cat-B Vref108kt (AT72/AT76/DH8D/DH8C/SF34/SAAB — GroupI non-Stage-4) / BIZ Cat-C Vref122kt (GLEX/GL5T/G650/GLF6/FA8X/CL30/CL35/C25A — GroupI Stage-4) / LIGHT Cat-A Vref72kt (C172/C182/PA28/SR22/DA40/BE36) / MIL Cat-D Vref140kt (C5/C17/C130/KC10/KC46/E3) per AIM 5-4-7 Vref-driven category gate · structurally distinct from APCH-CAT (ILS CAT-I/II/III precision instrument minima compliance — CVFP is the visual-only counterpart), STABLE-APP (1000ft/500ft gate gross-stabilisation checks — does not score landmark conformance), CDFA-VDP (NPA continuous-descent vertical-path conformance — instrument vertical guidance), CIRCLING (post-instrument-approach circle-to-land manoeuvre), APPR-MINS (minima publication only, no track), STEEP-APCH (high-glidepath physics envelope ≥ 3.5°), DECRAB (crosswind technique at flare) — CVFP is uniquely the VISUAL-LANDMARK-AS-PRIMARY-REFERENCE evaluator with per-procedure named waypoint sequence (Key Bridge / Memorial Bridge / Pentagon / 14th St Br / River Bend, etc.) and per-waypoint published crossing altitude ±tolerance · weather synthesis per ICAO Annex 3 / FAA AC 00-45H ceiling+visibility random with DOY-cycling / 65% clear / 25% partial / 10% IFR-ish biased by WX-MUL slider · procedure-snap great-circle ≤ SCOPE-NM with track-vs-ldgHdg alignment ≤ ALIGN-DEG (default 60° — visual procedures permit significant turn-final manoeuvring) + nearest-landmark-distance ≤ SCOPE · 6 drivers CORR-DEV lateral corridor deviation perpendicular to nearest landmark (tolerance scales 0.4-2.5NM with distance to runway) / ALT-DEV altitude vs published landmark altitude ±per-LM tolerance / CAT-BUST aircraft Vref category exceeds proc max (per AIM 5-4-7 Cat-A &lt; 91kt / B 91-120 / C 121-140 / D 141-165 / E ≥ 166) / GROUP-BUST procedure-specific Group I jet / Stage-4 noise cert / OpSpec C062 (~92% carrier coverage proxy) breach / WX-BUST current ceiling/visibility below procedure-specific minima (CANARSIE 5sm 3500ft / QUIET BRIDGE 5sm 4000ft / others 3sm 3500ft) / NOISE noise-sensitive procedure + heavy airframe penalty · composite max·0.66 + mean·0.34 × ADV-MUL · hard escalators CAT-BUST ≥ 80 → score min 85 / GROUP-BUST ≥ 80 → min 82 / WX-BUST ≥ 80 → min 80 / corridor &gt; 5NM → min 72 (lost the landmark sequence) · 6 tiers BREACH ≥ 80 rose multiple driver violations discontinue visual request vectors to instrument approach / CORRIDOR-LOSS ≥ 60 rose-pink lost landmark sequence re-acquire or transition to instrument / DEVIATE ≥ 40 amber altitude/track outside tolerance correct per chart profile / WATCH ≥ 22 sky on profile monitor next landmark / CONFORM &lt; 22 + VMC emerald continue visual / CLEAR slate VMC marginal no traffic on procedure yet · MapLibre overlay tier-coloured aircraft halo+pin+label + amber landmark waypoint markers with per-LM published altitude label + dashed sky-blue procedure track polyline (LM[0] → LM[N-1] → runway threshold) + tier-coloured dashed link aircraft→nearest-landmark for breach/corridor-loss/deviate tiers · Side panel 6-tier counter strip click-to-filter ALL + 5-cell summary In-CVFP/VMC-OK/Breach/Procs/μ-Score + 4 sliders ADV-MUL/WX-MUL/DOY/SCOPE-NM + 6-region chip filter (NE/MID/SE/MIDW/MTW/WEST) + per-procedure dropdown filter + 6 overlay toggles (Halo/Pin/Lbl/LM/Track/Link) + search by callsign/type/operator/proc · AIRCRAFT tier-sorted row stack with cs+type+tier-pill+Cat-pill + PROC/RWY/ALT/ΔALT/CORR/DIST/CEIL/VIS 8-cell + tier-coloured score bar + driver chips + tier-coloured advice citing regulatory ref + "at LM name idx X/N" caption · PROCEDURES per-CVFP active-traffic worst-tier-pill + Cat-Max/Group/Ceil-Min/Vis-Min/LMs 5-cell + WX state CEIL/VIS/VMC ✓✗ + airport+name + amber hazard-note banner (P-56 prohibited / Tobin Br 393ft / Tysons antenna farm / Trump Twr 1389ft / Sears Twr 1450ft) · LANDMARKS per-procedure waypoint listing with idx + name + published altitude + ±tolerance + RWY threshold row with elev+true-heading · METHOD regulatory-basis / VMC-gate / landmark-sequencing / category-group-restrictions / 6-driver-decomposition / 6-tier-ladder / distinct-from / references sections — CVFP is uniquely the landmark-tracking visual-approach compliance scorer answering the operational question "is this airframe RIGHT NOW within the published charted-landmark corridor at the published landmark altitudes with the required VMC and within the airframe-category / Group I / Stage-4 / OpSpec C062 eligibility band to legally continue this visual approach?"', showCvfp, ()=>{ const nv=!showCvfp; setShowCvfp(nv); lsSet('ft-cvfp', nv) }],
                ...
                ['AIDC · ATS Inter-facility Data Communication & OLDI Handoff Protocol Conformance Monitor · per-flight live evaluator of the inter-facility ATC↔ATC coordination message stack governing how each airborne flight is handed across adjacent ACC/UAC/Oceanic/Approach boundaries — distinct from FIR-LOAD (count-only sector load, no protocol dimension), FIR-CROSSINGS (geographic boundary catalogue only), CPDLC (pilot↔controller text datalink not ATC↔ATC), VDL-2/HFDL (physical-link air-ground bearer), PBCS (RCP/RSP certification framework), AMAN/E-AMAN (single-FIR arrival sequencer — though E-AMAN consumes AIDC for upstream metering windows), CLAM/RAM (cleared-level/route adherence ground safety-nets), MTCD/STCA (medium/short conflict probes), SWIM (information distribution backbone — AIDC uses it as a transport in newer profiles), CSC (call-sign confusion taxonomy) — AIDC is uniquely the INTER-FACILITY MESSAGE STACK conformance evaluator answering for each flight crossing an adjacent-ANSP boundary in the next ~15 min whether the canonical sequence ABI → (PAC) → EST → ACT → LAM → TOC → AOC has been emitted by the upstream centre, acknowledged by the downstream centre, and completed within deadline, with per-stage timer compliance, per-LAM responsiveness, REV churn rate, and Überlingen-precedent escalators · 11-entry canonical message catalogue ABI Advance Boundary Information T-COBT-900s / PAC Pre-Activation T-COBT-420s OLDI-only European overlay / EST Estimate T-COBT-420s / REV Revision on-change / ACT Activation T-COBT-180s / MAC Message for Abrogation on-cancel / LAM Logical Acknowledgement ≤4s respond-by (OLDI ed.4.3 §B.2) or ≤30s relaxed APAC / COD Coordination manual handoff / INF Information free-text / ROF Request On Frequency pilot-on-freq trigger OLDI extension / CDN Coordination Dialogue conditional revision OLDI §C / plus protocol envelope TOC Transfer of Control authority pivot T-COBT-30s + AOC Assumption of Control downstream accepts ≤120s post-COBT + OCM Oceanic Clearance Message NAT/NICS PAC overlay T-COBT-1200s · 15-pair bilateral catalogue EDUU↔EISN MUAC↔Shannon UIR NAT-E gateway OLDI / EDUU↔EGTT Maastricht↔London highest-volume EU pair OLDI / EDUU↔LFFF MUAC↔Paris OLDI / EDUU↔EDMM MUAC↔Munich OLDI / EDMM↔LSAS Munich↔Zurich UAC OLDI — Überlingen BFU AX001-1-2/02 precedent 2002-07-01 STCA outage + LAM/REV verbal-coordination failure 71 fatal / LFFF↔LECM Paris↔Madrid OLDI / LFFF↔LIRR Paris↔Rome OLDI / ZNY↔EISN NY-ARTCC↔Shannon NAT-E westbound oceanic exit NAT NTS profile / ZAK↔ZAN Oakland Oceanic↔Anchorage NICS Pacific FAA AIDC-NICS POM / ZAK↔RJJJ Oakland↔Fukuoka NICS PAC / RJJJ↔ZGZU Fukuoka↔Guangzhou ASIA/PAC AIDC-ICD ed.3.0 / WSJC↔VHHK Singapore↔Hong Kong ASIA/PAC / OMAE↔VIDP Emirates↔Delhi ASIA/PAC / YBBB↔WSJC Melbourne↔Singapore ASIA/PAC / ZMA↔MUFH Miami↔Habana CAR/SAM each with declared inter-facility capacity (movements/hr), bilateral LAM timer (4s OLDI vs 30s NAT/NICS/APAC), ABI/EST/ACT lead deadlines, profile-overlay (OLDI/ICAO/APAC/NICS/NAT), and accident precedent · 8 drivers ABI-LATE timer breach for Advance Boundary Info (s past T-COBT lead) ramp 0→300s / EST-LATE Estimate timer breach ramp 0→180s / ACT-LATE Activation timer breach ramp 0→120s / LAM-MISS OLDI §B.2 ≤4s ack missing or APAC ≤30s relaxed (count·severity·lamMul) / REV-CHURN revision/abrogation churn per leg 0-3 / TOC-GAP Transfer-of-Control issued without Assumption — unowned aircraft slice per Doc 4444 §10.4.5 / PROFILE bilateral profile field-set mismatch (OLDI PAC missing on European bilateral etc) / ESC Überlingen-style escalator (≥2 stages missing AND STCA-degraded proxy) score-min 92 · composite max·0.66 + mean·0.34 × ADV-MUL with hard escalators ESC score-min 92 Überlingen fall-back to verbal HF/SATVOICE per Doc 4444 §10.5 / LAM-MISS×2 score-min 80 OLDI §B.2 deadline breached re-emit EST + chase ack / TOC-without-AOC score-min 76 unowned aircraft slice query downstream / ABI not emitted at T-COBT lead score-min 60 coordination dialogue at risk / EST overdue score-min 50 partial sequence OLDI §A.6 / REV-churn ≥3 score-min 45 clearance instability consider CDN dialogue OLDI §C · 6 tiers BREACH ≥80 rose coordination dialogue failed verbal HF/SATVOICE fall-back do NOT release / DEGRADED ≥62 rose-pink partial handoff chase LAM re-emit EST log inc / LATE ≥42 amber one stage past timer recover with REV-LAM cycle / WATCH ≥22 sky nominal sequence monitor next stage trigger / CLEAN <22 emerald full sequence completed in envelope / IDLE slate flight not in any look-ahead bilateral window · MapLibre overlay tier-coloured aircraft halo+pin+label sized by score + per-bilateral diamond markers at boundary midpoint sized by declared capacity dashed sky-blue (active) or slate (idle) + bilateral ANSP-id label + dashed tier-coloured link line aircraft→boundary-midpoint for active flights · Side panel 6-tier counter strip click-to-filter ALL + 5-cell summary μ-SCORE / μ-LAM-latency s / LAM-MISS-count / BREACH+DEGRADED count / WORST callsign + 4 sliders ADV-MUL 50-200% / SCOPE NM 80-500 / LOOK min 5-30 (T-COBT look-ahead window) / LAM-MUL 50-200% + 6-region chip filter (EUR/NAT/NICS-PAC/APAC/CAR-SAM) + 6-profile chip filter (OLDI/ICAO/APAC/NICS/NAT) + 15-bilateral chip filter + 5 overlay toggles (HALO/PIN/LBL/BND/LNK) + search by callsign/type/bilateral-id + AIRCRAFT/BILATERAL/STAGES/METHOD tab switcher · AIRCRAFT tier-worst-first row stack with cs+type+bilateral-id-pill+profile-pill+tier-pill + T-COBT min / DIST NM / FL / REV count 4-cell + per-stage visual sequence strip emerald=acked amber=emitted-not-acked slate=not-due showing LAM latency per stage when present + tier-coloured score bar + 8-driver chips + advice line citing OLDI / Doc 9694 / Doc 4444 / BFU AX001-1-2/02 Überlingen precedent · BILATERAL per-pair row click-to-filter with bilateral-id-pill+profile-pill+region-pill + DECL-mins/h LAM-timer-s ABI-lead-m EST-lead-m 4-cell + μ-SCORE BREACH DEGRADED LAM-MISS 4-cell + upstream→downstream ANSP name + italic accident/operational precedent note · STAGES per-stage envelope across all flight×bilateral pairs in look-ahead window with stage-pill+full-name + EMIT% ACK% μ-LAT-s 3-cell + visualisation bar (emerald=acked + amber=emitted-not-acked) + canonical regulatory reference · METHOD definition + 11-entry message catalogue + 8-driver decomposition + 6-tier ladder + structurally-distinct-from list + comprehensive regulatory reference catalogue · References ICAO Doc 9694 ATS Data Link Applications Manual · Doc 4444 PANS-ATM §10 Coordination & Transfer · Doc 7030 RASPs · Annex 11 §3.7 ATS Coordination · Annex 10 Vol II §3.3 AFTN/AMHS · EUROCONTROL OLDI Specification ed.4.3 (2024) Annex A messages / Annex B LAM timer / Annex C COD · EUROCONTROL OLDI ICD-MNUC-3.0 Multi-NUC bilateral · FAA NAS-IR-25080004 ER-6 IFDD Inter-Facility Data Distribution · FAA AIDC-NICS Pacific Operations Manual (ZAK/ZAN/Tokyo) · FAA Order JO 7110.65 §8-5 Oceanic / §8-6 AIDC / §10-3 ATCSCC interfacility · FAA Order JO 7610.4 §8 oceanic/non-radar coordination · FAA Order JO 7210.633 NICS Pacific IO-Hbk · ICAO APAC AIDC-ICD ed.3.0 (CRG Bali 2018) · NAT NTS RNDSG/57 AIDC Pacific & NAT profile · ICAO Doc 9854 GATMOC §3.4 information sharing · Doc 9882 SWIM Manual · Doc 9931 CDM Manual §4 inter-ANSP info · Doc 9971 Pt II Ch.6 Sector Capacity & Coord · ED-153 Software SSAL for SNETs · ED-202A Airworthiness AISG · CANSO SoE-SMS 2019 · BFU AX001-1-2/02 Überlingen final 2004 (71 fatal 2002-07-01 inter-FIR coordination accident family driver — DFS Zürich vs Skyguide LSAS failure of LAM/REV verbal coordination during STCA outage) · BEA AF447 (informational — NAT inter-FIR oceanic handoff relevance) · NTSB AAR-91-08 LAX1493 · ICAO Cir 314 HF training §6 inter-unit coord. AIDC entry registered in Layers Routes & Flow category after HOLD, ft-aidc persisted preference.', showAidc, ()=>{ const nv=!showAidc; setShowAidc(nv); lsSet('ft-aidc', nv) }],
                ...
                ['TOPC · FMS perf data-entry audit · TOW/ZFW/FLEX/INTS/QNH/RWY cross-check vs load-sheet · catches MK1602 / EK407 / TOM263 / Comair 5191 GIGO class · per FAA InFO 06017 / 18004 · SAFO 18003 · EASA SIB 2017-13 · ICAO Doc 10020 EFB Manual', showTopc, ()=>{ const nv=!showTopc; setShowTopc(nv); lsSet('ft-topc', nv) }],
