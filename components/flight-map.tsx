          ...
          <div className="mt-16 mr-4 w-[min(92vw,420px)] max-h-[78vh] overflow-y-auto bg-slate-950/95 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl" onClick={e=>e.stopPropagation()}>
            ...
            {[
              ...
              {group:'Safety & Traffic', items:[
                ...
                ['CAST · CAST/SE Top-Accident-Category Susceptibility Monitor · per-airframe real-time scorer against the CAST/ICAO CICTT 8-category top-accident taxonomy LOC-I (Loss of Control In-flight) / CFIT (Controlled Flight Into Terrain) / RE (Runway Excursion) / MAC (Mid-Air Collision incl NMAC) / SCF-PP (System/Component Failure Powerplant) / ARC (Abnormal Runway Contact) / WSTRW (Weather/Wake/Windshear Encounter) / USOS (Undershoot/Overshoot) with phase-of-flight gating per CAST common-taxonomy phases GND/TXO/TOF/ICL/ENR/DST/APP/LDG · phase-gated scoring zeros categories irrelevant to current phase (RE/ARC only count APP/LDG/TOF/RTO, MAC only airborne, USOS only APP/LDG) · per-flight aggregate uses dominant-category model per CAST chain-of-events methodology · O(N²) nearest-traffic precompute drives MAC proximity score with opposite-level geometry · single-engine-class amplifier on SCF-PP · stall-margin/bank/upset proxies on LOC-I · 6 hard tiers per ICAO Doc 9859 SMS Risk Tolerability Matrix EXTREME ≥85 rose intolerable / HIGH ≥70 rose-pink tolerable w/mitigation / ELEVATED ≥50 amber / MODERATE ≥30 sky / LOW ≥12 emerald / NEGL <12 slate · MapLibre tier-coloured halo rings 7-22px by score + EXTREME/HIGH rose pins + category-coloured callsign+dominant-cat+tier labels · AIRCRAFT/CATEGORIES/PHASES tab switcher · AIRCRAFT tab sorted tier-worst-first then score desc with tier color stripe + cs+type+dominant-cat-pill+phase-pill+tier-pill + FL+kt+VS row + 8-cat breakdown chips muted-when-out-of-phase + tier-coloured advice citing per-category regulatory references · CATEGORIES tab per-CICTT-category with name + ac-count + mean score bar + ext/high count + regulatory reference italic · PHASES tab per-phase with ac-count + applicable-categories pill row showing the CAST phase-gate matrix · ADV-MUL/PROX-NM/MIN-FL/MAX-FL sliders + per-category chip filter ALL/LOC-I/CFIT/RE/MAC/SCF-PP/ARC/WSTRW/USOS + HALO/PIN/LBL toggles + search by callsign/cat/phase/operator/type · (CAST/ICAO Common Taxonomy Team CICTT Aviation Occurrence Categories rev 4.7 / ICAO Doc 9756 Pt IV App C / ICAO Doc 9859 SMS Manual ed.4 risk matrix / ICAO Doc 10011 Manual on Aeroplane UPRT / ICAO Doc 9870 sec 8 runway incursion prevention / ICAO Doc 9981 PANS-Aerodromes / FAA Order 8000.369C SMS / FAA CAST/SE Plan SE-3 SE-30 SE-201 SE-205 SE-209 SE-211 / AC 23-18 / AC 25-23 EGPWS / AC 25-9A engine failure / AC 90-23G Wake / AC 91-79B Runway Overrun Prevention / AC 120-55C TCAS / AC 120-71 Stable Approach / AC 120-111 UPRT / AC 00-54 Pilot Windshear Guide / AC 25.473 / FAR 25.901 / RTCA DO-185B TCAS-II / EASA Part-FCL App 9 UPRT / FSF Runway Excursion Risk Reduction Toolkit ed.2 / FSF ALAR Toolkit Briefing 8.1 / NTSB SR-87-02 SR-04-01 SR-14-02 SR-19-02 / NTSB SA-040 CFIT / Boeing Statistical Summary 2024 / IATA Safety Report 2024 sec 3)', showCast, ()=>{ const nv=!showCast; setShowCast(nv); lsSet('ft-cast', nv) }],
              ]},
